...
nonzero_entries_in_matrix.remove(2**n)

# Build
pb_ddts = {}
for p in nonzero_entries_in_matrix:
    pb_ddts[p] = matrix(2**n,2**m)
...
for p in nonzero_entries_in_matrix:
    print(f'Constraints for {p}-DDT')
    for i in range(pb_ddts[p].nrows()):
        for j in range(pb_ddts[p].ncols()):
            if pb_ddts[p][i][j] == 0:
...
milp.add_constraint(sum(x[i] for i in range(0,6)) >= 1)
# milp.add_constraint(sum(x[i] for i in range(3,6)) >= 1)
milp.add_constraint(x[8] == 1)
milp.set_objective(2*x[6] + 1*x[7])
milp.show()

print(f'Minimum value of the objective function {milp.solve()}')
