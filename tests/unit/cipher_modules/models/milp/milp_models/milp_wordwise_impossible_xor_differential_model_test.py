...
def test_build_wordwise_impossible_xor_differential_trail_model():
    ...
    milp._backward_cipher = backward_cipher.add_suffix_to_components(
...
def test_find_one_wordwise_impossible_xor_differential_trail_model():
    ...
    ciphertext = set_fixed_variables(
        component_id="cipher_output_1_32", constraint_type="equal", bit_positions=range(16), bit_values=(1,) + (0,) * 15
    )
    trail = milp.find_one_wordwise_impossible_xor_differential_trail(
        1, fixed_bits=[key], fixed_words=[plaintext, ciphertext]
    )
    assert trail["status"] == SATISFIABLE
    assert trail["components_values"][INPUT_PLAINTEXT]["value"] == "1003000000000000"
    assert trail["components_values"][INPUT_KEY]["value"] == "0000000000000000"
...
def test_find_one_wordwise_impossible_xor_differential_trail_model_with_fixed_components():
    ...
    trail = milp.find_one_wordwise_impossible_xor_differential_trail_with_chosen_components(
        ["mix_column_0_21"], fixed_bits=[key], fixed_words=[plaintext, ciphertext]
    )
    assert trail["status"] == SATISFIABLE
    assert trail["components_values"][INPUT_PLAINTEXT]["value"] == "1003000000000000"
    assert trail["components_values"][INPUT_KEY]["value"] == "0000000000000000"
    assert trail["components_values"]["cipher_output_1_32"]["value"] == "1000000000000000"
    assert trail["components_values"]["xor_0_36_backward"]["value"] == "2000000000000000"
    assert trail["components_values"]["intermediate_output_0_37_backward"]["value"] == "2000000000000000"
...
def test_find_one_wordwise_impossible_xor_differential_trail_with_fully_automatic_model():
    ...
    key_backward = set_fixed_variables(
        component_id="key_backward", constraint_type="equal", bit_positions=range(128), bit_values=(0,) * 128
    )
    ciphertext_backward = set_fixed_variables(
        component_id="cipher_output_1_32_backward",
        constraint_type="equal",
        bit_positions=range(16),
        bit_values=(1,) + (0,) * 15,
    )
    trail = milp.find_one_wordwise_impossible_xor_differential_trail_with_fully_automatic_model(
        fixed_bits=[key, key_backward], fixed_words=[plaintext, ciphertext_backward]
    )
    assert trail["status"] == SATISFIABLE
    assert trail["components_values"][INPUT_PLAINTEXT]["value"] == "1003000000000000"
    assert trail["components_values"][INPUT_KEY]["value"] == "0000000000000000"
...
def test_find_one_wordwise_impossible_xor_differential_trail_model_with_external_solver():
    ...
    trail = milp.find_one_wordwise_impossible_xor_differential_trail(
        1, fixed_bits=[key], fixed_words=[plaintext, ciphertext], external_solver_name="glpk_ext"
    )
    assert trail["status"] == SATISFIABLE
    assert trail["components_values"][INPUT_PLAINTEXT]["value"] == "1003000000000000"
    assert trail["components_values"][INPUT_KEY]["value"] == "0000000000000000"
...
def test_find_one_wordwise_impossible_xor_differential_trail_with_fully_automatic_model_with_external_solver():
    ...
    trail = milp.find_one_wordwise_impossible_xor_differential_trail_with_fully_automatic_model(
        fixed_bits=[key, key_backward], fixed_words=[plaintext, ciphertext_backward], external_solver_name="glpk_ext"
    )
    assert trail["status"] == SATISFIABLE
    assert trail["components_values"][INPUT_PLAINTEXT]["value"] == "1003000000000000"
    assert trail["components_values"][INPUT_KEY]["value"] == "0000000000000000"
