...
minizinc4 = MinizincXorDifferentialModelARXOptimized(speck4)

speck5 = SpeckBlockCipher(number_of_rounds=5, block_bit_size=32, key_bit_size=64)
minizinc5 = MinizincXorDifferentialModelARXOptimized(speck5)
...
def test_find_lowest_weight_xor_differential_trail():
    ...
    minizinc = MinizincXorDifferentialModelARXOptimized(speck)
    ...
    minizinc = MinizincXorDifferentialModelARXOptimized(tea)
    ...
    pr_weights_per_round = [{"min_bound": 2, "max_bound": 4},
                            ...
                            {"min_bound": 2, "max_bound": 4},
                            {"min_bound": 2, "max_bound": 4}]
    minizinc = MinizincXorDifferentialModelARXOptimized(speck5, probability_weight_per_round=pr_weights_per_round)
    ...
    minizinc = MinizincXorDifferentialModelARXOptimized(speck5, sat_or_milp="milp")
    result = minizinc.find_lowest_weight_xor_differential_trail(solver_name='Xor', fixed_values=fixed_variables_32_64)
    assert result["total_weight"] == 9

    minizinc = MinizincXorDifferentialModelARXOptimized(speck5, [0, 0, 0, 0, 0])
    result = minizinc.find_lowest_weight_xor_differential_trail(solver_name='Xor', fixed_values=fixed_variables_32_64)
    assert result["total_weight"] == 9

    speck = SpeckBlockCipher(number_of_rounds=4)
    minizinc = MinizincXorDifferentialModelARXOptimized(speck, sat_or_milp="milp")
    bit_positions_key = list(range(64))
    fixed_variables = [{'component_id': 'key',
                        'constraint_type': 'sum',
                        'bit_positions': bit_positions_key,
    ...
    tea = TeaBlockCipher(number_of_rounds=2)
    minizinc = MinizincXorDifferentialModelARXOptimized(tea, sat_or_milp="milp")
    result = minizinc.find_lowest_weight_xor_differential_trail(solver_name='Xor', fixed_values=fixed_variables_64_128)
    assert result["total_weight"] > 1

    raiden = RaidenBlockCipher(number_of_rounds=2)
    minizinc = MinizincXorDifferentialModelARXOptimized(raiden, sat_or_milp="milp")
    result = minizinc.find_lowest_weight_xor_differential_trail(solver_name='Xor', fixed_values=fixed_variables_64_128)
    assert result['total_weight'] == 6
    ...
    pr_weights_per_round = [{"min_bound": 2, "max_bound": 4},
                            ...
                            {"min_bound": 2, "max_bound": 4},
                            {"min_bound": 2, "max_bound": 4}]
    minizinc = MinizincXorDifferentialModelARXOptimized(speck5,
...
def test_get_probability_vars_from_key_schedule():
    minizinc = MinizincXorDifferentialModelARXOptimized(speck4)
