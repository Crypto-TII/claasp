...
def test_find_all_xor_differential_trails_with_fixed_weight():
    result = MZN5.find_all_xor_differential_trails_with_fixed_weight(
        5, solver_name=CPSAT, fixed_values=fixed_variables_32_64
    )
...
def test_find_all_xor_differential_trails_with_weight_at_most():
    result = MZN4.find_all_xor_differential_trails_with_weight_at_most(
        1, solver_name=CPSAT, fixed_values=fixed_variables_32_64
    )
    assert result[0]["total_weight"] > 1


def test_find_lowest_weight_xor_differential_trail():
    result = MZN5.find_lowest_weight_xor_differential_trail(solver_name=CPSAT, fixed_values=fixed_variables_32_64)
    assert result["total_weight"] == 9

    mzn = MznXorDifferentialModelARXOptimized(SPECK5, [0, 0, 0, 0, 0])
    result = mzn.find_lowest_weight_xor_differential_trail(solver_name=CPSAT, fixed_values=fixed_variables_32_64)
    assert result["total_weight"] == 9
    ...
    result = mzn.find_lowest_weight_xor_differential_trail(solver_name=CPSAT, fixed_values=fixed_variables)
    assert result["total_weight"] == 0

    tea = TeaBlockCipher(number_of_rounds=2)
    mzn = MznXorDifferentialModelARXOptimized(tea)
    result = mzn.find_lowest_weight_xor_differential_trail(solver_name=CPSAT, fixed_values=fixed_variables_64_128)
    ...
    solution = mzn.find_lowest_weight_xor_differential_trail(solver_name=CPSAT, fixed_values=fixed_variables_32_64)
...
def test_find_lowest_weight_for_short_xor_differential_trail():
    ...
    result = MZN4.find_lowest_weight_xor_differential_trail(solver_name=CPSAT, fixed_values=fixed_variables_32_64)
...
def test_find_min_of_max_xor_differential_between_permutation_and_key_schedule():
    ...
    result = mzn.find_min_of_max_xor_differential_between_permutation_and_key_schedule(
        fixed_values=fixed_variables_32_64, solver_name=CPSAT
    )
    assert result["total_weight"] == 5
