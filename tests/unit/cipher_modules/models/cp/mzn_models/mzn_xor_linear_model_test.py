...
from claasp.cipher_modules.models.minizinc.minizinc_models.minizinc_xor_linear_model import MinizincXorLinearModel


def test_and_xor_linear_probability_lat():
    simon = SimonBlockCipher()
    minizinc = MinizincXorLinearModel(simon)

    assert minizinc.and_xor_linear_probability_lat(2) == [2, 1, 0, 1, 0, 1, 0, -1]


def test_final_xor_linear_constraints():
    speck = SpeckBlockCipher(block_bit_size=32, key_bit_size=64, number_of_rounds=4)
    minizinc = MinizincXorLinearModel(speck)
    minizinc.build_xor_linear_trail_model(-1)

    assert minizinc.final_xor_linear_constraints(-1)[:-1] == \
...
def test_find_all_xor_linear_trails_with_fixed_weight():
    ...
    minizinc = MinizincXorLinearModel(speck)
    trails = minizinc.find_all_xor_linear_trails_with_fixed_weight(1)
...
def test_find_all_xor_linear_trails_with_weight_at_most():
    speck = SpeckBlockCipher(block_bit_size=8, key_bit_size=16, number_of_rounds=3)
    minizinc = MinizincXorLinearModel(speck)
    trails = minizinc.find_all_xor_linear_trails_with_weight_at_most(0, 1)
...
def test_find_lowest_weight_xor_linear_trail():
    speck = SpeckBlockCipher(block_bit_size=32, key_bit_size=64, number_of_rounds=4)
    minizinc = MinizincXorLinearModel(speck)
    trail = minizinc.find_lowest_weight_xor_linear_trail()
...
def test_find_one_xor_linear_trail():
    speck = SpeckBlockCipher(block_bit_size=32, key_bit_size=64, number_of_rounds=4)
    minizinc = MinizincXorLinearModel(speck)
    trail = minizinc.find_one_xor_linear_trail()
...
def test_find_one_xor_linear_trail_with_fixed_weight():
    speck = SpeckBlockCipher(block_bit_size=32, key_bit_size=64, number_of_rounds=4)
    minizinc = MinizincXorLinearModel(speck)
    trail = minizinc.find_one_xor_linear_trail_with_fixed_weight(3)
    assert str(trail['cipher']) == 'speck_p32_k64_o32_r4'
...
def test_fix_variables_value_xor_linear_constraints():
    speck = SpeckBlockCipher(block_bit_size=32, key_bit_size=64, number_of_rounds=4)
    minizinc = MinizincXorLinearModel(speck)
    assert minizinc.fix_variables_value_xor_linear_constraints(
...
def test_input_xor_linear_constraints():
    speck = SpeckBlockCipher(block_bit_size=32, key_bit_size=64, number_of_rounds=4)
    minizinc = MinizincXorLinearModel(speck)
    declarations, constraints = minizinc.input_xor_linear_constraints()
    ...
    minizinc = MinizincXorLinearModel(fancy)
    declarations, constraints = minizinc.input_xor_linear_constraints()
