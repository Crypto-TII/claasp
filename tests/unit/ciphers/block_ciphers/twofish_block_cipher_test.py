...
def test_twofish_block_cipher():
    ...
    assert cipher.evaluate_vectorized_gpu([key, plaintext], evaluate_api=True) == ciphertext
    ...
    assert two_fish.evaluate_vectorized_gpu([key, plaintext], evaluate_api=True) == ciphertext
