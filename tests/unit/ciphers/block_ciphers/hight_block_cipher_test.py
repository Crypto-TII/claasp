...
def test_hight_block_cipher():
    ...
    assert hight.evaluate_vectorized_gpu([plaintext, key], evaluate_api=True) == 0x1055ddee99ba66e0
    ...
    assert hight.evaluate_vectorized_gpu([plaintext, key], evaluate_api=True) == 0x2b8b6b285d2d0e9c
    ...
    assert hight.evaluate_vectorized_gpu([plaintext, key], evaluate_api=True) == 0x3b25d694326c4375