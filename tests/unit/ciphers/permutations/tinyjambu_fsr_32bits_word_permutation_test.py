from claasp.ciphers.permutations.tinyjambu_fsr_32bits_word_permutation import TinyJambuFSRWordBasedPermutation


def test_tinyjambu_fsr_32bits_word_permutation():
    tinyjambu = TinyJambuFSRWordBasedPermutation()
    assert tinyjambu.family_name == 'tinyjambu_fsr_word_based'
    assert tinyjambu.type == 'permutation'
    assert tinyjambu.number_of_rounds == 20
    assert tinyjambu.id == 'tinyjambu_fsr_word_based_p128_k128_o128_r20'
    assert tinyjambu.component_from(0, 0).id == 'constant_0_0'
    ...
    #assert tinyjambu.evaluate_vectorized_gpu([plaintext, key], evaluate_api=True) == ciphertext
