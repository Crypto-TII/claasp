...
def test_keccak_sbox_permutation():
    keccak = KeccakSboxPermutation()
    assert keccak.family_name == 'keccak_sbox'
    assert keccak.type == 'permutation'
    assert keccak.number_of_rounds == 24
    assert keccak.word_bit_size == 64
    assert keccak.id == 'keccak_sbox_p1600_o1600_r24'
    assert keccak.component_from(0, 0).id == 'xor_0_0'
    assert keccak.component_from(23, 0).id == 'xor_23_0'

    keccak = KeccakSboxPermutation(number_of_rounds=4, word_size=64)
    assert keccak.number_of_rounds == 4
    assert keccak.id == 'keccak_sbox_p1600_o1600_r4'
    assert keccak.component_from(3, 0).id == 'xor_3_0'

    keccak = KeccakSboxPermutation(number_of_rounds=1)
