...
generatorOptions(char** streamFile)
{
	...
	while ( (option < 0) || (option > NUMOFGENERATORS) ) {
		option = 0; //displayGeneratorOptions(); force file as input
		switch( option ) {
			case 0:
...
void
chooseTests(char * statiscal_test_options)
{
	int		i;
...
//	else {
		...
		for ( i=1; i<=NUMOFTESTS; i++ )  {
			//scanf("%1d", &testVector[i]);
		 	testVector[i] = (int) (statiscal_test_options[i-1]-'0');
        }
		//printf("\n\n");
//	}
}
...
void
fileBasedBitStreams(char *streamFile, int mode)
{
	FILE	*fp;
	//int		mode;
...
void
openOutputStreams(int option, int numOfBitStreams)
{
	int		i, numOfOpenFiles = 0;
	char	freqfn[200], summaryfn[200], statsDir[200], resultsDir[200];
	
	sprintf(freqfn, "%s/experiments/%s/freq.txt", WORKING_DIR, generatorDir[option]);
	if ( (freqfp = fopen(freqfn, "w")) == NULL ) {
		printf("\t\tMAIN:  Could not open freq file: <%s>", freqfn);
		exit(-1);
	}
	sprintf(summaryfn, "%s/experiments/%s/finalAnalysisReport.txt", WORKING_DIR, generatorDir[option]);
	...
	for( i=1; i<=NUMOFTESTS; i++ ) {
		if ( testVector[i] == 1 ) {
			sprintf(statsDir, "%s/experiments/%s/%s/stats.txt", WORKING_DIR, generatorDir[option], testNames[i]);
			sprintf(resultsDir, "%s/experiments/%s/%s/results.txt", WORKING_DIR, generatorDir[option], testNames[i]);
...
void
invokeTestSuite(int option, char *streamFile, int input_file_format)
{
	...
	switch( option ) {
		case 0:
			fileBasedBitStreams(streamFile, input_file_format);
			break;
		case 1:
			lcg();
			break;
