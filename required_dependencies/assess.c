...
main(int argc, char *argv[])
{
	...
	if ( argc != 6 ) {
		printf("Usage: %s <input file> <stream length> <number of bit streams> <input file format> <statistical tests options>\n", argv[0]);
		printf("   <input file> is the file of bit stream(s) to be processed. It should be absolute path\n");
		printf("   <stream length> is the length of the individual bit stream(s) to be processed\n");
		printf("   <number of bit streams> is the number of bit stream(s) to be processed\n");
		printf("   <input file format> the format of <input file>. 0 to be ASCII and 1 to be BINARY\n");
		printf("   <statistical tests options> is a binary string of size 15. This string is used to specify a set of statistical tests we want to run.\n");
		return 0;
	}
	...
	option = generatorOptions(&(argv[1]));
	chooseTests(argv[5]);
	fixParameters();
	openOutputStreams(option, atoi(argv[3]));
	invokeTestSuite(option, argv[1], atoi(argv[4]));
	//invokeTestSuite(option, argv[2]);
	fclose(freqfp);
...
partitionResultFile(int numOfFiles, int numOfSequences, int option, int testNameID)
{
	...
	sprintf(resultsDir, "%s/experiments/%s/%s/results.txt", WORKING_DIR, generatorDir[option], testNames[testNameID]);
	...
	for ( i=0; i<numOfFiles; i++ ) {
		if ( i < 10 )
			sprintf(s[i], "%s/experiments/%s/%s/data%1d.txt", WORKING_DIR, generatorDir[option], testNames[testNameID], i+1);
		else if (i < 100)
			sprintf(s[i], "%s/experiments/%s/%s/data%2d.txt", WORKING_DIR, generatorDir[option], testNames[testNameID], i+1);
		else
			sprintf(s[i], "%s/experiments/%s/%s/data%3d.txt", WORKING_DIR, generatorDir[option], testNames[testNameID], i+1);
	}
	numread = 0;
	m = numOfFiles/20;
	if ( (numOfFiles%20) != 0 )
		...
		if ( testVector[i] ) {
			...
			if ( ((i == TEST_CUSUM) && testVector[TEST_CUSUM] ) ||
				...
				for ( k=0; k<numOfFiles; k++ ) {
					if ( k < 10 )
						sprintf(s, "%s/experiments/%s/%s/data%1d.txt", WORKING_DIR, generatorDir[option], testNames[i], k+1);
					else if ( k < 100 )
						sprintf(s, "%s/experiments/%s/%s/data%2d.txt", WORKING_DIR, generatorDir[option], testNames[i], k+1);
					else
						sprintf(s, "%s/experiments/%s/%s/data%3d.txt", WORKING_DIR, generatorDir[option], testNames[i], k+1);
				...
				sprintf(s, "%s/experiments/%s/%s/results.txt", WORKING_DIR, generatorDir[option], testNames[i]);
				generalSampleSize = computeMetrics(s,i);
			}
		}
	}
