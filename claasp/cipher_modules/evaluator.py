...
def evaluate_with_intermediate_outputs_continuous_diffusion_analysis(
    ...
    python_code_string = (
        code_generator.generate_python_code_string_for_continuous_diffusion_analysis(
            cipher, verbosity
        )
    )
    python_code_string = python_code_string.replace(
        "def evaluate(input):",
        "def evaluate(input, sbox_precomputations, sbox_precomputations_mix_columns):",
    )

    f_module = ModuleType("evaluate_continuous_diffusion_analysis")
    exec(python_code_string, f_module.__dict__)

    return f_module.evaluate(
        cipher_input, sbox_precomputations, sbox_precomputations_mix_columns
    )
