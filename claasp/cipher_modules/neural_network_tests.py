...
class NeuralNetworkDistinguisher:
    def __init__(self, cipher):
        super(NeuralNetworkDistinguisher, self).__init__()
        self.cipher = cipher
    ...
    def neural_staged_training(self, data_generator, starting_round, neural_network=None, training_samples=10 ** 7,
        ...
        while acc >= threshold and nr < self.cipher.number_of_rounds:
            ...
            #                                  num_epochs)
            acc = self.train_neural_distinguisher(data_generator, nr, neural_network, training_samples, testing_samples,
                                                  num_epochs)
            accuracies[nr] = acc
            nr += 1
        return accuracies
