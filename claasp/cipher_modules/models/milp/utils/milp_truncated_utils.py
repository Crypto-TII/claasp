...
def generate_incompatiblity_constraints_for_component(
    model, model_type, x, x_class, backward_component, include_all_components
):
...
def generate_all_incompatibility_constraints_for_fully_automatic_model(
    model, model_type, x, x_class, include_all_components
):
...
def fix_variables_value_deterministic_truncated_xor_differential_constraints(
    milp_model, model_variables, fixed_variables=[]
):
    constraints = []
...
def _generate_value_exclusion_constraints(milp_model, model_variables, fixed_variable, prefix, suffix):
    ...
    if sum(fixed_variable["bit_values"]) == 0:
        constraints.append(
            sum(
                model_variables[f"{fixed_variable['component_id']}{prefix}_{i}{suffix}"]
