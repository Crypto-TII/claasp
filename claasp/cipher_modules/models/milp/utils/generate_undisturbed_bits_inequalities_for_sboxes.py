...
from claasp.cipher_modules.models.milp.utils.utils import (
    delete_espresso_dictionary,
    generate_product_of_sum_from_espresso,
    output_espresso_dictionary,
)

undisturbed_bit_sboxes_inequalities_file_name = (
    "dictionary_that_contains_inequalities_for_sboxes_with_undisturbed_bits.obj"
)
undisturbed_bit_sboxes_inequalities_file_path = os.path.join(
    MILP_AUXILIARY_FILE_PATH, undisturbed_bit_sboxes_inequalities_file_name
)


def _encode_transition(delta_in, delta_out, verbose):
    encoded_in = [_ for j in delta_in for _ in map(int, f"{j:02b}")]
    encoded_out = [_ for j in delta_out for _ in map(int, f"{j:02b}")]
    if verbose:
...
def get_transitions_for_single_output_bit(sbox, valid_points, verbose=False):
    ddt_with_undisturbed_bits_transitions = [
        _encode_transition(input, output, verbose) for input, output in valid_points
    ]
    n = sbox.input_size()
    ...
    for position in range(n):
        ...
        for encoding_bit in range(2):
            valid_points[position][encoding_bit] = [
                transition[: 2 * n] + transition[2 * (n + position) + encoding_bit]
                for transition in ddt_with_undisturbed_bits_transitions
            ]
