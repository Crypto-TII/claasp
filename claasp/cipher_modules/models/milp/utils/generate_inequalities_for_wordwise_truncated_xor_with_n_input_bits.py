...
wordwise_truncated_input_pattern_inequalities_file_path = os.path.join(
    MILP_AUXILIARY_FILE_PATH, input_patterns_file_name
)
wordwise_truncated_xor_inequalities_between_n_input_bits_file_path = os.path.join(
    MILP_AUXILIARY_FILE_PATH, xor_n_inputs_file_name
)
...
def generate_valid_points_input_words(wordsize=4, max_pattern_value=3):
    ...
    if max_pattern_value == 3:
        list_of_possible_inputs = [(0, 0)] + [(1, i) for i in range(1, 1 << wordsize)] + [(2, 0)] + [(3, 0)]

        for delta, zeta in list_of_possible_inputs:
            tmp = "".join(format(delta, "0" + str(bit_len) + "b") + format(zeta, "0" + str(wordsize) + "b"))
...
def generate_valid_points_for_xor_between_n_input_words(wordsize=4, number_of_words=2):
    ...
    list_of_possible_inputs = [(0, 0)] + [(1, i) for i in range(1, 1 << wordsize)] + [(2, -1)] + [(3, -2)]
    ...
    for input in itertools.product(list_of_possible_inputs, repeat=number_of_words):
        ...
        for summand in range(number_of_words - 2):
            tmp_delta[summand + 1], tmp_zeta[summand + 1] = get_valid_points_for_wordwise_xor(
                tmp_delta[summand], tmp_zeta[summand], delta[summand + 1], zeta[summand + 1]
            )
        ...
        tmp = (
            "".join(
                format(delta[i], "0" + str(bit_len) + "b")
                + format(zeta[i] if (delta[i] == 1) else 0, "0" + str(wordsize) + "b")
