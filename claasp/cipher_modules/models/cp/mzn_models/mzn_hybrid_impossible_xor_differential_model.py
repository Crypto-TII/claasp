...
class MznHybridImpossibleXorDifferentialModel(MznImpossibleXorDifferentialModel):
    ...
    def build_hybrid_impossible_xor_differential_trail_model(
        ...
        if probabilistic:
            ...
            inverse_variables, inverse_constraints = self.build_improbable_backward_model(
            ...
            inverse_variables, inverse_constraints = self.build_impossible_backward_model(
                backward_components, clean=False
            )
    ...
    def build_improbable_forward_model(self, forward_components, clean=False):
        ...
        for component in forward_components:
            if check_if_implemented_component(component):
                variables, constraints = self.propagate_deterministically(
                    component, key_schedule=(component.id in key_ids), probabilistic=True
                )
    ...
    def build_improbable_backward_model(self, backward_components, clean=True):
        ...
        for component in backward_components:
            if check_if_implemented_component(component):
                variables, constraints = self.propagate_deterministically(
                    component, key_schedule=(component.id in key_ids), inverse=True, probabilistic=True
                )
    ...
    def _generate_wordwise_incompatibility_constraint(self, component):
        if self.sbox_size:
            ...
            if self._cipher.is_spn() or self._output_is_aligned_with_sboxes(path_indices):
                ...
                intermediate_output_bit_positions = itertools.combinations(
                    range(len(path_indices.keys()) * self.sbox_size), self.sbox_size
                )
            ...
            for bit_positions, suffix in itertools.product(intermediate_output_bit_positions, ["", "inverse_"]):
                ...
                constraint += "/\\".join(
                    [
                        f"({component.id}[{i}]+inverse_{component.id}[{i}]={suffix + component.id}[{i}])"
                ...
                constraint += "/\\" + "/\\".join(
                    [
                        f"({suffix + component.id}[{i}] = {suffix + component.id}[{bit_positions[0]}])"
                        for i in bit_positions[1:]
                    ]
                )
    ...
    def final_impossible_constraints(
        self, number_of_rounds, initial_round, middle_round, final_round, intermediate_components, probabilistic=False
    ):
        """
        ...
        if intermediate_components:
            ...
            for component in cipher.get_all_components():
                ...
                if "output" in component.id:
                    ...
                    if (
                        self.get_component_round(component.id) == middle_round - 1
                        and component.id not in key_schedule_components_ids
                        and component.description == ["round_output"]
                    ):
        ...
        cp_constraints.append(
            f"constraint ({bitwise_incompatibility_constraint}) \\/ ({wordwise_incompatibility_constraint});"
        )
    ...
    def find_all_impossible_xor_differential_trails(
        ...
        """
        self.build_hybrid_impossible_xor_differential_trail_model(
            fixed_values,
            number_of_rounds,
            initial_round,
            middle_round,
                ...
                if probabilistic:
                    if "AND" in component.description[0] or component.description[0] == "OR":
                        ...
                        update_and_or_ddt_valid_probabilities(
                            and_already_added, component, cp_declarations, valid_probabilities
                        )
    ...
    def propagate_deterministically(
        self, component, key_schedule=False, wordwise=False, inverse=False, probabilistic=False
    ):
        if not wordwise:
            if component.type == SBOX:
                if key_schedule and probabilistic:
                    ...
                    variables, constraints, sbox_cache = (
                        component.cp_hybrid_deterministic_truncated_xor_differential_constraints(
                            self.sbox_cache, inverse, self.sboxes_component_number_list
                        )
                    )
                    self.sbox_cache = sbox_cache
    ...
    def format_component_value(self, component_id, string):
        ...
        value = [
            "." if x == 0 else str(x) if x < 2 else "?" if x == 2 else str(x % 7 + 2)
            for x in ast.literal_eval(value[3:])
        ]
    ...
    def _parse_solver_output(
        self, output_to_parse, number_of_rounds, initial_round, middle_round, final_round, probabilistic=False
    ):
        if probabilistic:
    ...
    def solve(
        ...
        if solver_process.returncode >= 0:
            ...
            solve_time, memory, components_values, total_weight = self._parse_solver_output(
                solver_output, number_of_rounds, initial_round, middle_round, final_round, probabilistic
            )
            if probabilistic:
