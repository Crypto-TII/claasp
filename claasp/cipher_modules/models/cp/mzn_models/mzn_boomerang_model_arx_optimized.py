...
class MznBoomerangModelARXOptimized(MznXorDifferentialModelARXOptimized):
    def __init__(self, cipher, top_end_ids, bottom_start_ids, middle_ids, window_size_list=None, sat_or_milp="sat"):
        ...
        self.top_graph, self.bottom_graph = split_cipher_graph_into_top_bottom(
            cipher, self.top_end_ids, self.bottom_start_ids
        )
    ...
    def reduce_cipher(new_cipher, original_cipher, graph):
        for round_number in range(new_cipher.number_of_rounds):
            MznBoomerangModelARXOptimized.remove_components_not_in_graph(
                new_cipher, original_cipher, round_number, graph
            )
    ...
    def objective_generator(mzn_top_cipher, mzn_bottom_cipher):
        objective_string = []
        modular_addition_concatenation = (
            "++".join(mzn_top_cipher.probability_vars) + "++" + "++".join(mzn_bottom_cipher.probability_vars)
        )
        ...
        mzn_top_cipher.mzn_output_directives.append(
    ...
    def create_boomerang_model(self, fixed_variables_for_top_cipher, fixed_variables_for_bottom_cipher):
        self.differential_model_top_cipher = MznXorDifferentialModelARXOptimized(
            self.top_cipher,
            window_size_list=[0 for _ in range(self.top_cipher.number_of_rounds)],
            sat_or_milp="sat",
            include_word_operations_mzn_file=False,
        )
        ...
        self.differential_model_bottom_cipher = MznXorDifferentialModelARXOptimized(
            self.bottom_cipher,
            window_size_list=[0 for _ in range(self.bottom_cipher.number_of_rounds)],
            sat_or_milp="sat",
            include_word_operations_mzn_file=False,
        )
        ...
        self.differential_model_bottom_cipher.extend_model_constraints(
            MznBoomerangModelARXOptimized.objective_generator(
                self.differential_model_top_cipher, self.differential_model_bottom_cipher
            )
        )
        self.differential_model_bottom_cipher.extend_model_constraints(
