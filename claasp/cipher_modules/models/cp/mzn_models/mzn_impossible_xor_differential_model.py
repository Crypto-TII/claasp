...
class MznImpossibleXorDifferentialModel(MznDeterministicTruncatedXorDifferentialModel):
    ...
    def fix_variables_value_constraints(self, fixed_variables=[], step='full_model'):
        ...
        cp_constraints = []
        if fixed_variables == []:
            fixed_variables.append(set_fixed_variables('plaintext', 'not_equal', list(range(self._cipher.output_bit_size)), [0]*self._cipher.output_bit_size))
            fixed_variables.append(set_fixed_variables('inverse_' + self._cipher.get_all_components_ids()[-1], 'not_equal', list(range(self._cipher.output_bit_size)), [0]*self._cipher.output_bit_size))
