...
class MznImpossibleXorDifferentialModel(MznDeterministicTruncatedXorDifferentialModel):
    ...
    def build_impossible_backward_model(self, backward_components, clean=True):
        ...
        if clean:
            ...
            inverse_variables, inverse_constraints = (
                self.clean_inverse_impossible_variables_constraints_with_extensions(
    ...
    def build_impossible_xor_differential_trail_with_extensions_model(
        self, fixed_variables, number_of_rounds, initial_round, middle_round, final_round, intermediate_components=True
    ):
        """
        ...
        for n_r in list(range(initial_round - 1, middle_round)) + list(range(final_round, number_of_rounds)):
            forward_components.extend(self._cipher.components_in_round(n_r))

        backward_components = []
        for n_r in list(range(initial_round - 1)) + list(range(middle_round - 1, final_round)):
            backward_components.extend(inverse_cipher.components_in_round(number_of_rounds - 1 - n_r))

        components_to_link = []
        for component in self.inverse_cipher.all_components():
            ...
            if comp_r == initial_round - 2 or comp_r == final_round - 1:
                for id_link in component.input_id_links:
                    if self.get_component_round(id_link) > comp_r:
                        for input_component in self.inverse_cipher.all_components():
        ...
        deterministic_truncated_xor_differential.extend(
            self.final_impossible_constraints_with_extensions(
    ...
    def build_impossible_xor_differential_trail_model(
        ...
        if fully_automatic:
            ...
            backward_components = inverse_cipher.all_components()
        else:
            forward_components = []
    ...
    def clean_constraints(self, set_of_constraints, initial_round, middle_round, final_round, fully_automatic=False):
        ...
        if fully_automatic:
            ...
            for r in range(initial_round - 1, middle_round):
                forward_components.extend([component.id for component in self._cipher.components_in_round(r)])
            backward_components = []
            for r in range(number_of_rounds - final_round, number_of_rounds - middle_round + 1):
                backward_components.extend(
                    ["inverse_" + component.id for component in self.inverse_cipher.components_in_round(r)]
                )
            _, key_ids = self.extract_key_schedule()
            components_to_keep = (
                forward_components
                + backward_components
                + key_ids
                + ["inverse_" + id_link for id_link in key_ids]
                + ["array["]
                + [SOLVE_SATISFY]
            )
        ...
        if initial_round > 1:
            for component in self._cipher.components_in_round(initial_round - 2):
    ...
    def clean_inverse_impossible_variables_constraints(
        self, backward_components, inverse_variables, inverse_constraints
    ):
        for component in backward_components:
            inverse_variables, inverse_constraints = self.set_inverse_component_id_in_constraints(
                component, inverse_variables, inverse_constraints
            )
        inverse_variables, inverse_constraints = self.clean_repetitions_in_constraints(
            inverse_variables, inverse_constraints
        )
        return inverse_variables, inverse_constraints

    def clean_inverse_impossible_variables_constraints_with_extensions(
        self, backward_components, inverse_variables, inverse_constraints
    ):
        key_components, key_ids = self.extract_key_schedule()
        constant_components, constant_ids = self.extract_constants()
    ...
    def clean_repetitions_in_constraints(self, inverse_variables, inverse_constraints):
        for c in range(len(inverse_constraints)):
            ...
            while "cipher_output" in inverse_constraints[c][start:]:
                ...
                inverse_constraints[c] = (
                    inverse_constraints[c][:new_start] + "inverse_" + inverse_constraints[c][new_start:]
                )
                start = new_start + 9
            start = 0
    ...
    def final_impossible_constraints_with_extensions(
        self, number_of_rounds, initial_round, middle_round, final_round, intermediate_components
    ):
        """
        ...
        """
        _, key_schedule_components_ids = self.extract_key_schedule()
        ...
        for id_link in self._cipher.all_components_ids():
            if (
                id_link not in key_schedule_components_ids
                and self.get_component_round(id_link)
                in list(range(initial_round - 1, middle_round)) + list(range(final_round, number_of_rounds))
                and "constant" not in id_link
                and "output" in id_link
            ):
                new_constraint = new_constraint + f'"{id_link} = "++ show({id_link})++ "\\n" ++ "0" ++ "\\n" ++'
            if (
                id_link not in key_schedule_components_ids
                and self.get_component_round(id_link)
                in list(range(initial_round - 1)) + list(range(middle_round - 1, final_round))
                and "constant" not in id_link
                and "output" in id_link
            ):
                new_constraint = (
                    new_constraint + f'"inverse_{id_link} = "++ show(inverse_{id_link})++ "\\n" ++ "0" ++ "\\n" ++'
                )
    ...
    def final_impossible_constraints(
        self, number_of_rounds, initial_round, middle_round, final_round, intermediate_components, fully_automatic
    ):
        """
        Constraints for output and incompatibility.
        ...
        def show_constraints_intermediate_components(
            component_list, key_schedule_components_ids, current_constraint, current_incompatibility_constraint
        ):
            for component in component_list:
                if component.type != CONSTANT and component.id not in key_schedule_components_ids:
                    ...
                    for id_link, bit_positions in zip(input_id_links, input_bit_positions):
                        ...
                        input_bit_size += len(bit_positions)
                        current_constraint = (
                            current_constraint + f'"{id_link} = "++ show({id_link})++ "\\n" ++ "0" ++ "\\n" ++'
                        )
                    current_constraint = (
                        current_constraint
                        + f'"inverse_{component_id} = "++ show(inverse_{component_id})++ "\\n" ++ "0" ++ "\\n" ++'
                    ...
                    for i in range(input_bit_size):
                        current_incompatibility_constraint += (
                            f"({component_inputs[i]}+inverse_{component_id}[{i}]=1) \\/ "
                        )
        ...
        def show_constraints(key_schedule_components_ids, current_constraint, current_incompatibility_constraint):
            for component in self._cipher.all_components():
                if "output" in component.id and component.id not in key_schedule_components_ids:
                    current_constraint = (
                        current_constraint + f'"{component.id} = "++ show({component.id})++ "\\n" ++ "0" ++ "\\n" ++'
                    )
                    current_constraint = (
                        current_constraint
                        + f'"inverse_{component.id} = "++ show(inverse_{component.id})++ "\\n" ++ "0" ++ "\\n" ++'
                    ...
                    for i in range(component.output_bit_size):
                        current_incompatibility_constraint += (
                            f"({component.id}[{i}]+inverse_{component.id}[{i}]=1) \\/ "
                        )
            return current_constraint, current_incompatibility_constraint
        ...
        if final_round == self._cipher.number_of_rounds:
            ...
            cipher_outputs = ["key"]
            for component in self.inverse_cipher.components_in_round(self._cipher.number_of_rounds - final_round):
                if "output" in component.id:
        ...
        if fully_automatic:
            ...
            for element in cipher_outputs:
                if element != "key":
                    new_constraint = (
                        f'{new_constraint}"inverse_{element} = "++ show(inverse_{element}) ++ "\\n" ++ "0" ++ "\\n" ++'
                    )
            if intermediate_components:
                component_list = cipher.all_components()
                new_constraint, incompatibility_constraint = show_constraints_intermediate_components(
                    component_list, key_schedule_components_ids, new_constraint, incompatibility_constraint
                )
            else:
                new_constraint, incompatibility_constraint = show_constraints(
                    key_schedule_components_ids, new_constraint, incompatibility_constraint
                )
        else:
            for element in cipher_inputs:
                new_constraint = f'{new_constraint}"{element} = "++ show({element}) ++ "\\n" ++'
            ...
            if intermediate_components:
                component_list = cipher.components_in_round(middle_round - 1)
                new_constraint, incompatibility_constraint = show_constraints_intermediate_components(
                    component_list, key_schedule_components_ids, new_constraint, incompatibility_constraint
                )
            else:
    ...
    def find_lowest_complexity_impossible_xor_differential_trail(
        self,
        number_of_rounds=None,
        fixed_values=[],
        solver_name=SOLVER_DEFAULT,
        ...
        self._model_constraints.append(
            f"solve minimize count(plaintext, 2) + count(inverse_{self._cipher.all_components_ids()[-1]}, 2);"
        )
    ...
    def find_one_impossible_xor_differential_cluster(
        self,
        number_of_rounds=None,
        fixed_values=[],
        solver_name=SOLVER_DEFAULT,
        ...
        self._model_constraints.remove("solve satisfy;")
        self._model_constraints.append(
            f"solve maximize count(plaintext, 2) + count(inverse_{self._cipher.all_components_ids()[-1]}, 2);"
        )
    ...
    def find_one_impossible_xor_differential_trail_with_extensions(
        self,
        number_of_rounds=None,
        fixed_values=[],
        solver_name=SOLVER_DEFAULT,
        ...
        if fixed_variables == []:
            fixed_variables.append(
                set_fixed_variables(
                    "plaintext",
                    "not_equal",
                    list(range(self._cipher.output_bit_size)),
                    [0] * self._cipher.output_bit_size,
                )
            )
            fixed_variables.append(
                set_fixed_variables(
                    "inverse_" + self._cipher.all_components_ids()[-1],
                    "not_equal",
                    list(range(self._cipher.output_bit_size)),
                    [0] * self._cipher.output_bit_size,
                )
    ...
    def component_from_id(self, id_link, curr_cipher):
        for component in curr_cipher.all_components():
    ...
    def get_direct_component_correspondance(self, forward_component):
        for inverse_component in self.inverse_cipher.all_components():
    ...
    def input_impossible_constraints_with_extensions(self, number_of_rounds, initial_round, middle_round, final_round):
        cp_constraints = []
        cp_declarations = [
        ...
        for component in forward_components:
            for id_link in component.input_id_links:
                ...
                if (
                    ...
                    and input_component is not None
                ):
        ...
        backward_components = []
        for component in inverse_cipher.all_components():
            comp_r = self.get_component_round(component.id)
        ...
        for component in backward_components:
            ...
            if "output" in component.type:
                ...
                if (
                    self.get_component_round(component.id) == final_round - 1
                    or self.get_component_round(component.id) == initial_round - 2
                ):
    ...
    def input_impossible_constraints(self, number_of_rounds, middle_round, fully_automatic=False):
        ...
        inverse_cipher = self.inverse_cipher
        if fully_automatic:
            forward_components = cipher.all_components()
            backward_components = inverse_cipher.all_components()
        else:
            forward_components = []
            for r in range(middle_round):
        ...
        cp_declarations.extend(
            [
                f"array[0..{bit_size - 1}] of var 0..2: inverse_{input_};"
    ...
    def _parse_solver_output(
        self, output_to_parse, number_of_rounds, initial_round, middle_round, final_round, fully_automatic
    ):
        components_values, memory, time = self.parse_solver_information(output_to_parse, True, True)
        all_components = [*self._cipher.inputs]
        if fully_automatic:
            all_components.extend([component.id for component in self._cipher.all_components()])
            all_components.extend(["inverse_" + component.id for component in self.inverse_cipher.all_components()])
        ...
        for component_id in all_components:
            ...
            for j, string in enumerate(output_to_parse):
                if f"{component_id}" in string and "inverse_" not in component_id + string:
                    ...
                    self.add_solution_to_components_values(
                        component_id, component_solution, components_values, j, output_to_parse, solution_number, string
                    )
        ...
        if solver_process.returncode >= 0:
            ...
            if model_type in [
                ...
                solver_time, memory, components_values = self._parse_solver_output(
                    solver_output, number_of_rounds, initial_round, middle_round, final_round, fully_automatic
                )
                total_weight = 0
            else:
                solver_time, memory, components_values, total_weight = self._parse_solver_output(
                    solver_output, number_of_rounds, initial_round, middle_round, final_round, fully_automatic
                )
