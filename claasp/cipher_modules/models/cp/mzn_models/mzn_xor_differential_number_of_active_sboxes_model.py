...
class MznXorDifferentialNumberOfActiveSboxesModel(MznModel):
    ...
    def build_xor_differential_trail_first_step_model(
        self, weight=-1, fixed_variables=[], nmax=2, repetition=1, possible_sboxes=0
    ):
        """
    ...
    def create_xor_component(self, component1, component2, nmax):
        ...
        if new_numb_of_inp < nmax and all_inputs != [] and not_add != 1:
            ...
            xor_components_dictionaries = [
                component.as_python_dictionary() for component in self.list_of_xor_components
            ]
    ...
    def final_xor_differential_first_step_constraints(self, weight=-1):
        ...
        cp_constraints = [
            f"constraint number_of_active_sBoxes = {inputs};",
        ...
        new_constraint = (
            'output[show(number_of_active_sBoxes) ++ "\\n" ++ " '
            'table_of_solution_length = "++ show(table_of_solutions_length)'
            + "".join([f' ++ "\\n" ++ " {input_[0]} = "++ show({input_[0]})' for input_ in self.input_sbox])
            + "];"
        )
    ...
    def get_new_xor_input_links_and_positions(self, all_inputs, new_numb_of_inp):
        ...
        for i in range(new_numb_of_inp):
            ...
            if new_input_name not in input_id_link:
                ...
                input_bit_positions[input_index] += [
                    int(new_input_bit_positions) * self.word_size + j for j in range(self.word_size)
                ]
                input_index = input_index + 1
            else:
                for j, present_input in enumerate(input_id_link):
                    if present_input == new_input_name:
                        input_bit_positions[j] += [
                            int(new_input_bit_positions) * self.word_size + word_size
    ...
    def get_xor_all_inputs(self, component1, component2):
        ...
        for id_link, bit_positions in zip(input_id_links_1, input_bit_positions_1):
            old_all_inputs.extend(
                [
                    f"{id_link}[{bit_positions[j * self.word_size] // self.word_size}]"
    ...
    def input_xor_differential_first_step_constraints(self, possible_sboxes=0):
        ...
        cp_declarations.extend(
            [
                f"array[0..{bit_size // self.word_size - 1}] of var 0..1: {input_};"
