...
from claasp.cipher_modules.models.minizinc.minizinc_model import MinizincModel, solve_satisfy, constraint_type_error
...
class MinizincXorLinearModel(MinizincModel):
    ...
    def and_xor_linear_probability_lat(self, numadd):
        ...
        EXAMPLES::
            ...
            sage: from claasp.cipher_modules.models.minizinc.minizinc_models.minizinc_xor_linear_model import MinizincXorLinearModel
            sage: from claasp.ciphers.block_ciphers.simon_block_cipher import SimonBlockCipher
            sage: simon = SimonBlockCipher()
            sage: minizinc = MinizincXorLinearModel(simon)
            sage: minizinc.and_xor_linear_probability_lat(2)
            [2, 1, 0, 1, 0, 1, 0, -1]
        """
        lat = []
    ...
    def branch_xor_linear_constraints(self):
        ...
        EXAMPLES::
            ...
            sage: minizinc = MinizincXorLinearModel(speck)
            sage: minizinc.branch_xor_linear_constraints()
    ...
    def build_xor_linear_trail_model(self, weight=-1, fixed_variables=[]):
        ...
        EXAMPLES::
            ...
            sage: minizinc.build_xor_linear_trail_model(-1, fixed_variables)
        """
    ...
    def final_xor_linear_constraints(self, weight):
        ...
        EXAMPLES::
            ...
            sage: fixed_variables = [set_fixed_variables('plaintext', 'not_equal', list(range(32)), integer_to_bit_list(0, 32, 'little'))]
            sage: minizinc.build_xor_linear_trail_model(-1, fixed_variables)
            sage: minizinc.final_xor_linear_constraints(-1)[:-1]
    ...
    def find_all_xor_linear_trails_with_fixed_weight(self, fixed_weight, fixed_values=[], solver_name=SOLVER_DEFAULT, solve_with_API=False):
        """
        ...
        EXAMPLES::
            ...
            sage: minizinc = MinizincXorLinearModel(speck)
            sage: trails = minizinc.find_all_xor_linear_trails_with_fixed_weight(1) # long
            sage: len(trails)
            12
            ...
            sage: trails = minizinc.find_all_xor_linear_trails_with_fixed_weight(2, fixed_values=[key])
            sage: len(trails)
            8
        """
        start = tm.time()
    ...
    def find_all_xor_linear_trails_with_weight_at_most(self, min_weight, max_weight=64,
                                                       fixed_values=[], solver_name=SOLVER_DEFAULT, solve_with_API=False):
        """
        ...
        EXAMPLES::
            ...
            sage: speck = SpeckBlockCipher(block_bit_size=8, key_bit_size=16, number_of_rounds=3)
            sage: minizinc = MinizincXorLinearModel(speck)
            sage: trails = minizinc.find_all_xor_linear_trails_with_weight_at_most(0, 1)
            sage: len(trails)
            13
            ...
            sage: from claasp.cipher_modules.models.utils import set_fixed_variables
            sage: key = set_fixed_variables('key', 'not_equal', list(range(16)), [0] * 16)
            sage: trails = minizinc.find_all_xor_linear_trails_with_weight_at_most(0, 3, fixed_values=[key])
            sage: len(trails)
            73
        """
        start = tm.time()
    ...
    def find_lowest_weight_xor_linear_trail(self, fixed_values=[], solver_name=SOLVER_DEFAULT, solve_with_API=False):
        """
        ...
        EXAMPLES::
            ...
            sage: minizinc = MinizincXorLinearModel(speck)
            sage: trail = minizinc.find_lowest_weight_xor_linear_trail()
            ...
            sage: trail = minizinc.find_lowest_weight_xor_linear_trail(fixed_values=[key])
            sage: trail['total_weight']
            '3.0'
        """
        start = tm.time()
    ...
    def find_one_xor_linear_trail(self, fixed_values=[], solver_name=SOLVER_DEFAULT, solve_with_API=False):
        """
        ...
        EXAMPLES::
            ...
            sage: speck = SpeckBlockCipher(block_bit_size=32, key_bit_size=64, number_of_rounds=4)
            sage: minizinc = MinizincXorLinearModel(speck)
            sage: minizinc.find_one_xor_linear_trail() # random
            ...
            sage: minizinc.find_one_xor_linear_trail(fixed_values=[key]) # random
        """
        start = tm.time()
        self.build_xor_linear_trail_model(0, fixed_values)
        end = tm.time()
    ...
    def find_one_xor_linear_trail_with_fixed_weight(self, fixed_weight=-1, fixed_values=[], solver_name=SOLVER_DEFAULT, solve_with_API=False):
        """
        ...
        EXAMPLES::
            ...
            sage: speck = SpeckBlockCipher(block_bit_size=32, key_bit_size=64, number_of_rounds=4)
            sage: minizinc = MinizincXorLinearModel(speck)
            sage: trail = minizinc.find_one_xor_linear_trail_with_fixed_weight(3)
            sage: trail['total_weight']
            '3.0'
            ...
            sage: from claasp.cipher_modules.models.utils import set_fixed_variables
            sage: key = set_fixed_variables('key', 'not_equal', list(range(16)), [0] * 16)
            sage: trail = minizinc.find_one_xor_linear_trail_with_fixed_weight(3, fixed_values=[key])
            sage: trail['total_weight']
            '3.0'
        """
        start = tm.time()
    ...
    def fix_variables_value_xor_linear_constraints(self, fixed_variables=[]):
        ...
        EXAMPLES::
            ...
            sage: speck = SpeckBlockCipher(block_bit_size=32, key_bit_size=64, number_of_rounds=4)
            sage: minizinc = MinizincXorLinearModel(speck)
            sage: minizinc.fix_variables_value_xor_linear_constraints([set_fixed_variables('plaintext', 'equal', list(range(4)), integer_to_bit_list(5, 4, 'big'))])
            ['constraint plaintext_o[0] = 0 /\\ plaintext_o[1] = 1 /\\ plaintext_o[2] = 0 /\\ plaintext_o[3] = 1;']
            sage: minizinc.fix_variables_value_xor_linear_constraints([set_fixed_variables('plaintext', 'not_equal', list(range(4)), integer_to_bit_list(5, 4, 'big'))])
            ['constraint plaintext_o[0] != 0 \\/ plaintext_o[1] != 1 \\/ plaintext_o[2] != 0 \\/ plaintext_o[3] != 1;']
        """
        cp_constraints = []
    ...
    def input_xor_linear_constraints(self):
        ...
        EXAMPLES::
            ...
            sage: speck = SpeckBlockCipher(block_bit_size=32, key_bit_size=64, number_of_rounds=4)
            sage: minizinc = MinizincXorLinearModel(speck)
            sage: minizinc.input_xor_linear_constraints()
    ...
    def weight_xor_linear_constraints(self, weight):
        ...
        EXAMPLES::
            ...
            sage: speck = SpeckBlockCipher(block_bit_size=32, key_bit_size=64, number_of_rounds=4)
            sage: minizinc = MinizincXorLinearModel(speck)
            sage: minizinc.weight_xor_linear_constraints(10)
