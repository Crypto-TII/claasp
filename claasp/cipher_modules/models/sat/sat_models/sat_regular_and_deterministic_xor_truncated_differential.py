...
class SatRegularAndDeterministicXorTruncatedDifferential(SatModel):
    ...
    def get_connecting_constraints(self):
        ...
        for regular_component_in_the_border in set(regular_components_in_the_border):
            ...
            print(regular_component_in_the_border)
            for idx in range(regular_component_in_the_border_claasp.output_bit_size):
                connecting_constraints = sat_utils.get_cnf_bitwise_truncate_constraints(
                        f'{regular_component_in_the_border}_{idx}', f'{regular_component_in_the_border}_{idx}_0', f'{regular_component_in_the_border}_{idx}_1'
                    )

                self._model_constraints.extend(connecting_constraints)

                self._variables_list.extend([f'{regular_component_in_the_border}_{idx}', f'{regular_component_in_the_border}_{idx}_0', f'{regular_component_in_the_border}_{idx}_1'])
    ...
    def unknown_variables_constraints(self, number_of_unknown_variables):
        ...
        set_to_be_minimized = []
        #[f"{INPUT_PLAINTEXT}_{i}_0" for i in range(self._cipher.inputs_bit_size[self._cipher.inputs.index(INPUT_PLAINTEXT)])]
        set_to_be_minimized.extend([bit_id for bit_id in self._variables_list if bit_id.startswith(cipher_output_id) and bit_id.endswith("_0")])
        #import ipdb;
        #ipdb.set_trace()
    ...
    def find_one_xor_regular_truncated_differential_trail_with_fixed_weight(
        ...
        solution = self.solve(
            "XOR_REGULAR_DETERMINISTIC_DIFFERENTIAL", solver_name=solver_name
        )
