...
class SatXorDifferentialModel(SatModel):
    ...
    def build_xor_differential_trail_and_checker_model_at_intermediate_output_level(
        ...
        The checker model verifies that there is at least one message pair satisfying the found trail at every
        intermediate output.
    ...
    def _fixed_component_constraints(self, plaintext, ciphertext):
        """
        Return a list of variables to fix plaintext to ``plaintext``, ciphertext to ``ciphertext`` and key to 0.
    ...
    def compute_xor_differential_weight(
        ...
        """
        Return the aggregated differential weight and the XOR differential trails within [lower_weight, upper_weight].

        The aggregated weight accounts for every trail found whose weight lies between ``lower_weight`` and
        ``upper_weight``. By default, the search is set in the single-key setting. If the search does not have any
        solution the function will return (None, []).

        INPUT:

        - ``plaintext`` -- **string**; hexadecimal representation of the plaintext for which to look for trails
        - ``ciphertext`` -- **string**; hexadecimal representation of the ciphertext for which to look for trails
        ...
        def create_file_names():
            ...
            cipher_info = (
                f"{self._cipher}_{plaintext}_{ciphertext}__geq{geq}_leq{upper_weight}"
                f"__{solver_name}solver{join_and_sanitize_strings(options)}"
            )
        ...
        while solution["total_weight"] is not None:
            solution["building_time_seconds"] = end_building_time - start_building_time
            solution["test_name"] = (
                f"find_xor_differential_trails_with_fix_plaintext_{plaintext}_ciphertext_{ciphertext}"
                f"_weight_geq_{lower_weight}_leq_{upper_weight}"
            )
            solutions_list.append(solution)
            if log:
    ...
    def find_one_xor_differential_trail(
        self, fixed_values=[], lower_bound=None, upper_bound=None, solver_name=solvers.SOLVER_DEFAULT, options=None
    ):
        """
        ...
        - ``solver_name`` -- **string** (default: `CRYPTOMINISAT_EXT`); the name of the solver
        - ``options`` -- **list[str]** (default: `None`); list for solver options when it is external
        ...
        EXAMPLES::
            ...
            sage: sat = SatXorDifferentialModel(speck)
            sage: trail = sat.find_one_xor_differential_trail(lower_bound=3, upper_bound=3)
            sage: trail['total_weight']
            3.0
            ...
            ....:     bit_values=[0]*64
            ....: )
            sage: trail = sat.find_one_xor_differential_trail(fixed_values=[key], lower_bound=3, upper_bound=3)
            sage: trail['total_weight']
            3.0
        """
        ...
        start_building_time = time.time()
        self.build_xor_differential_trail_model(weight=upper_bound, fixed_variables=fixed_values)
