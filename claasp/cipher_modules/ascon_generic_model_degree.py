...
# [0, x1, 1 + x2, x3, x4] --> [y0, y1, y2, y3, y4]
def anf_ascon_sbox_with_const_0eq(model, x, y):
    ...
    copy_x4 = model.addVars(list(range(7)), vtype=GRB.BINARY)
    for i in range(7):
        model.addConstr(x[3] >= copy_x4[i])
    model.addConstr(sum(copy_x4[i] for i in range(7)) >= x[3])

    a = model.addVars(list(range(7)), vtype=GRB.BINARY, name="a")

    ########### Constraints for each of the 5 equations (ai for each occurence of products)
    model.addConstr(a[0] == copy_x4[0]), model.addConstr(a[0] == copy_x1[0])  # x4x1
    model.addConstr(a[1] == copy_x2[0]), model.addConstr(a[1] == copy_x1[1])  # x2x1
    model.addConstr(
        y[0] >= a[0] + a[1] + copy_x3[0] + copy_x2[1])  # >= means const 1 need to be add

    model.addConstr(a[2] == copy_x3[1]), model.addConstr(a[2] == copy_x2[2])  # x3x2
    model.addConstr(a[3] == copy_x3[2]), model.addConstr(a[3] == copy_x1[2])  # x3x1
    model.addConstr(a[4] == copy_x1[3]), model.addConstr(a[4] == copy_x2[3])  # x1x2
    model.addConstr(
        y[1] >= a[2] + a[3] + a[4] + copy_x4[1] + copy_x2[4])

    model.addConstr(a[5] == copy_x4[2]), model.addConstr(a[5] == copy_x3[3])  # x4x3
    model.addConstr(
        y[2] == a[5] + copy_x4[3] + copy_x2[5] + copy_x1[4])

    model.addConstr(
        y[3] >= copy_x4[4] + copy_x3[4] + copy_x2[6] + copy_x1[5])

    model.addConstr(a[6] == copy_x4[5]), model.addConstr(a[6] == copy_x1[6])  # x4x1
    model.addConstr(
        y[4] == a[6] + copy_x4[6] + copy_x3[5] + copy_x1[7])
...
def anf_ascon_sbox_0eq(model, x, y):
    ...
    for i in range(10):
        model.addConstr(x[0] >= copy_x1[i])
    model.addConstr(sum(copy_x1[i] for i in range(10)) >= x[0])
    ...
    for i in range(7):
        model.addConstr(x[2] >= copy_x3[i])
    model.addConstr(sum(copy_x3[i] for i in range(7)) >= x[2])
    ...
    model.addConstr(a[1] == copy_x2[0]), model.addConstr(a[1] == copy_x1[1])  # x2x1
    model.addConstr(
        y[0] == a[0] + a[1] + copy_x3[0] + copy_x2[1] + copy_x1[2])  # >= means const 1 need to be add

    model.addConstr(a[2] == copy_x3[1]), model.addConstr(a[2] == copy_x2[2])  # x3x2
    model.addConstr(a[3] == copy_x3[2]), model.addConstr(a[3] == copy_x1[3])  # x3x1
    model.addConstr(a[4] == copy_x1[4]), model.addConstr(a[4] == copy_x2[3])  # x1x2
    model.addConstr(
        y[1] == a[2] + a[3] + a[4] + copy_x4[1] + copy_x3[3] + copy_x2[4] + copy_x1[5])

    model.addConstr(a[5] == copy_x4[2]), model.addConstr(a[5] == copy_x3[4])  # x4x3
    model.addConstr(
        y[2] >= a[5] + copy_x4[3] + copy_x2[5] + copy_x1[6])

    model.addConstr(
        y[3] == copy_x4[4] + copy_x3[5] + copy_x2[6] + copy_x1[7])

    model.addConstr(a[6] == copy_x4[5]), model.addConstr(a[6] == copy_x1[8])  # x4x1
    model.addConstr(
        y[4] == a[6] + copy_x4[6] + copy_x3[6] + copy_x1[9])
...
def anf_ascon_sbox_1eq(model, x, y):
    ...
    copy_x4 = model.addVars(list(range(6)), vtype=GRB.BINARY)
    for i in range(6):
        model.addConstr(x[3] >= copy_x4[i])
    model.addConstr(sum(copy_x4[i] for i in range(6)) >= x[3])
    ...
    model.addConstr(a[4] == copy_x1[3]), model.addConstr(a[4] == copy_x2[3])  # x1x2
    model.addConstr(
        y[1] >= a[2] + a[3] + a[4] + copy_x4[1] + copy_x3[3] + copy_x2[4] + copy_x1[4])

    model.addConstr(a[5] == copy_x4[2]), model.addConstr(a[5] == copy_x3[4])  # x4x3
    model.addConstr(
        y[2] >= a[5] + copy_x4[3] + copy_x2[5] + copy_x1[5])

    model.addConstr(
        y[3] >= copy_x2[6] + copy_x1[6])

    model.addConstr(a[6] == copy_x4[4]), model.addConstr(a[6] == copy_x1[7])  # x4x1
    model.addConstr(
        y[4] == a[6] + copy_x4[5] + copy_x3[5])
...
def term_enumeration(rounds, flag, target, degree):
    model = Model()
    model.Params.LogToConsole = 0
    # model.setParam("PoolSolutions", 2000000)
    # model.setParam(GRB.Param.PoolSearchMode, 2)

    S = model.addVars(list(range(256)), vtype=GRB.BINARY)
    ...
    ks = model.addVar()
    model.addConstr(ks == sum(X[rounds - 1][i] for i in range(320)))  # The last X is the output
    ...
    for i in range(128):
        ks_cube += S[128 + i]
    model.setObjective(ks_cube, GRB.MAXIMIZE)
    ...
    #     # print("y:", y)
    #     tmp = []
    #     for index, v in enumerate(values[:256]):
    #         if v == 1:
    #             tmp.append(index)
...
def rate_anf(rounds):
    ...
    for sbox in range(2):  # 64
        for i in range(5):
            # print("monomials in y" + str(64*i + sbox) + ":")
            term_enumeration(rounds, flag, 64 * i + sbox, degree)
            print("degree of y" + str(64 * i + sbox) + " : " + str(degree[64 * i + sbox]))
        print("\n")
