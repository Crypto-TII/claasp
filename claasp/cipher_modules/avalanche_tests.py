...
class AvalancheTests:
    ...
    def avalanche_tests(self, number_of_samples=5, avalanche_dependence_uniform_bias=0.05,
        ...
        intermediate_output_names = self.add_intermediate_output_components_id_to_dictionary(self.cipher.get_all_components())
        diffusion_tests = {"input_parameters": {
            ...
            "test_results": self.init_dictionary_test_results(intermediate_output_names)}
        ...
        for criterion_name in parameters.keys():
            for index, input_name in enumerate(self.cipher.inputs):
                for intermediate_output_name in list(intermediate_output_names.keys()):
                    if parameters[criterion_name][0]:
                        self.add_intermediate_output_values_to_dictionary(criterion_name, intermediate_output_names,
                        ...
                        all_output_vectors, largest_round_criterion_not_satisfied = \
                            self.calculate_regular_difference(criterion_name, criterion, intermediate_output_names, parameters,
    ...
    def init_dictionary_test_results(self, dict_intermediate_output_names):
        dict_test_results = {}
        for input_name in self.cipher.inputs:
    ...
    def is_output(self, component):
        return component.type == INTERMEDIATE_OUTPUT or component.type == CIPHER_OUTPUT


    def add_intermediate_output_components_id_to_dictionary(self, components):
        intermediate_output_names = {}
        for component in components:
            if self.is_output(component):
    ...
    def add_intermediate_output_values_to_dictionary(self, criterion_name, dict_intermediate_output_names,
        ...
        dict_test_results["input_parameters"][f'{intermediate_output_name}_{criterion_name}_input_bit_size'] = \
            self.cipher.inputs_bit_size[index]
    ...
    def calculate_regular_difference(self, criterion_name, dict_criterion, dict_intermediate_output_names, dict_parameters,
                                     dict_test_results, input_name, intermediate_output_name):
        all_output_vectors = {}
    ...
    def calculate_average_difference(self, all_output_vectors, criterion_name, dict_parameters, dict_test_results, input_name,
                                     intermediate_output_name):
    ...
    def calculate_worst_input_differences(self, criterion_name, largest_round_criterion_not_satisfied,
                                          dict_test_results, input_name, intermediate_output_name):
        max_round_criterion_not_satisfied = max(
            largest_round_criterion_not_satisfied.values(), default=self.cipher.number_of_rounds)
    ...
    def avalanche_probability_vectors(self, nb_samples):
        ...
        inputs = self.generate_random_inputs(nb_samples)
        evaluated_inputs = evaluator.evaluate_vectorized(self.cipher, inputs, intermediate_outputs=True, verbosity=False)
        input_bits_to_analyse = self.cipher.get_all_inputs_bit_positions()
        for index_of_specific_input, specific_input in enumerate(self.cipher.inputs):  # where the diff is injected
            for input_diff in input_bits_to_analyse[specific_input]:
                intermediate_avalanche_probability_vectors = self.generate_avalanche_probability_vectors(
    ...
    def generate_random_inputs(self, nb_samples):
        ...
        for i in range(len(self.cipher.inputs)):
            inputs.append(np.random.randint(256,
                                            size=(math.ceil(self.cipher.inputs_bit_size[i] / 8), nb_samples),
                                            dtype=np.uint8))

        return inputs


    def generate_avalanche_probability_vectors(self, dict_intermediate_output_names, inputs,
                                               evaluated_inputs, input_diff, index_of_specific_input):
        inputs_prime = self.generate_inputs_prime(index_of_specific_input, input_diff, inputs)
        evaluated_inputs_prime = evaluator.evaluate_vectorized(self.cipher, inputs_prime,
    ...
    def generate_inputs_prime(self, index_of_specific_input, input_diff, inputs):
        inputs_prime = []
        for input_index in range(len(self.cipher.inputs)):
            if input_index == index_of_specific_input:
                diff = (1 << (self.cipher.inputs_bit_size[input_index] - 1 - input_diff))
                diff_vectorized = np.array(
                    np.frombuffer(int.to_bytes(diff, math.ceil(self.cipher.inputs_bit_size[input_index] / 8), byteorder='big'),
    ...
    def compute_criterion_from_avalanche_probability_vectors(self, all_avalanche_probability_vectors,
        ...
        """
        intermediate_output_names = self.add_intermediate_output_rounds_id_to_dictionary()
        criterion = {}
            ...
            for output_tag in all_avalanche_probability_vectors[input_tag].keys():
                ...
                for input_diff in range(len(all_avalanche_probability_vectors[input_tag][output_tag])):
                    ...
                    for number_of_occurrence in range(
                        ...
                        self.set_vector_dependence(criterion, input_diff, input_tag, number_of_occurrence, output_tag, vector)
                        self.set_vector_dependence_uniform(avalanche_dependence_uniform_bias, criterion, input_diff,
                                                      input_tag, number_of_occurrence, output_tag, vector)
                        self.set_vector_weight(criterion, input_diff, input_tag, number_of_occurrence, output_tag, vector)
                        self.set_vector_entropy(criterion, input_diff, input_tag, number_of_occurrence, output_tag, vector)

        return criterion


    def set_vector_entropy(self, criterion, input_diff, input_tag, number_of_occurrence, output_tag, vector):
    ...
    def set_vector_weight(self, criterion, input_diff, input_tag, number_of_occurrence, output_tag, vector):
        criterion[input_tag][output_tag][input_diff][number_of_occurrence]["avalanche_weight_vectors"] = vector


    def set_vector_dependence_uniform(self, avalanche_dependence_uniform_bias, criterion, input_diff,
                                      input_tag, number_of_occurrence, output_tag, vector):
    ...
    def set_vector_dependence(self, criterion, input_diff, input_tag, number_of_occurrence, output_tag, vector):
        vector_dependence = [1 if proba != 0 else 0 for proba in vector]
        criterion[input_tag][output_tag][input_diff][number_of_occurrence][
            "avalanche_dependence_vectors"] = vector_dependence

    def add_intermediate_output_rounds_id_to_dictionary(self):
