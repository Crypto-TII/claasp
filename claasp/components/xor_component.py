...
class Xor(Component):
    ...
    def cms_constraints(self):
        ...
        for i, output_id in enumerate(output_ids):
            ...
            operands.extend(input_ids[i::self.output_bit_size])
    ...
    def sat_constraints(self):
        ...
        constraints = []
        for i, output_id in enumerate(output_ids):
            result_bit_ids = [f"inter_{j}_{output_id}" for j in range(self.description[1] - 2)] + [output_id]
            constraints.extend(sat_utils.cnf_xor_seq(result_bit_ids, input_ids[i::self.output_bit_size]))
    ...
    def sat_bitwise_deterministic_truncated_xor_differential_constraints(self):
        ...
        input_ids = [input_ids for input_ids in zip(input_ids_0, input_ids_1)]
        output_ids = [output_ids for output_ids in zip(output_ids_0, output_ids_1)]
        constraints = []
        for i, output_id in enumerate(output_ids):
            result_ids = [
                (f"inter_{j}_{self.id}_{i}_0", f"inter_{j}_{self.id}_{i}_1") for j in range(self.description[1] - 2)
            ] + [output_id]
            constraints.extend(sat_utils.cnf_xor_truncated_seq(result_ids, input_ids[i::self.output_bit_size]))
    ...
    def smt_constraints(self):
        ...
        constraints = []
        for i, output_id in enumerate(output_ids):
            operation = smt_utils.smt_xor(input_ids[i::self.output_bit_size])
