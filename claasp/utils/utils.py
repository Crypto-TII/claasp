...
def pprint_dictionary_to_file(dictionary, name_file):
    ...
    EXAMPLES::
        ...
        sage: analysis = cipher.analyze_cipher(tests_configuration)
        sage: pprint_dictionary_to_file(analysis['diffusion_tests']['input_parameters'], f"{dir_path}/test_json")
        sage: import os.path
        sage: os.path.isfile(f"{dir_path}/test_json")
        True

        sage: import os
        sage: os.remove(f"{dir_path}/test_json")
    """
