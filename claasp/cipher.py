...
class Cipher:
    ...
    def train_neural_distinguisher(self, data_generator, starting_round, neural_network, training_samples=10**7, testing_samples=10**6, pipeline = True):
                                                        verbose=False):
        """
        ...
        - ``neural_network`` -- **keras model** (default: `None`); the neural network to use for distinguishing, either a custom one or one
        ...
        EXAMPLES::
            ...
            sage: from claasp.ciphers.block_ciphers.speck_block_cipher import SpeckBlockCipher
            sage: from neural_network_tests import get_differential_dataset, get_neural_network
            sage: cipher = SpeckBlockCipher()
            sage: input_differences = [0x400000, 0]
            sage: data_generator = lambda nr, samples: get_differential_dataset(cipher, input_differences, nr = nr, samples = samples) 
            sage: neural_network = get_neural_network('gohr_resnet', input_size = 64, word_size = 16)
            sage: cipher.train_neural_distinguisher(data_generator, starting_round = 5, neural_network)
        """
