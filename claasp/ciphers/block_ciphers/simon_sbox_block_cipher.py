...
# ----------------------
# Simon's nonlinear round function is f(x) = ((x <<< 1) & (x <<< 8)) ^ (x <<< 2), where "<<<"
# is a cyclic left rotation of the whole word and "&"/"^" act bitwise. Because the two operands
# of the AND are DIFFERENT rotations of the SAME word, computing the AND bit-by-bit couples
# together bits that live at different positions of x -- it is not, on its face, a per-position
# lookup the way an ordinary S-box is.
#
# It becomes one once the AND is isolated from the rest of f: this SBOX table implements ONLY
# the AND part, (x <<< 1) & (x <<< 8); the outer XOR with (x <<< 2) is applied afterwards by a
# plain XOR component in f() below (see the comment there), exactly as in the plaintext formula.
#
# Bit j of (x <<< 1) is x_{j+1}, and bit j of (x <<< 8) is x_{j+8} (indices mod word_size). So for
# 8 consecutive output positions j, j+1, ..., j+7 the AND needs the 8+1 = 9 bits of x at positions
# j+1, j+2, ..., j+8, j+9 -- because bit (j+7)+8 = j+15 = (j+8)+7, i.e. each output bit's "x <<< 8"
# operand is the NEXT output bit's "x <<< 1" operand. Writing v_k = x_{j+1+k} for k = 0..8 (9 bits),
# the AND part at output position j+k is simply v_k & v_{k+1} for k = 0..7 -- an 8-bit result that
# depends on exactly 9 input bits, hence the 512 = 2**9 entries below (SBOX has 512 entries, NOT
# 256 -- it is addressed by a genuine 9-bit index, not truncated or wrapped in any way). Concretely,
# SBOX[v_0 v_1 ... v_8] (9-bit binary index, MSB-first) = (v_0&v_1)(v_1&v_2)...(v_6&v_7)(v_7&v_8)
# (8-bit binary output, MSB-first).
#
# f() below builds exactly this: for each 8-bit output chunk it selects the 9 input positions
# input_positions_pattern (+ an 8-bit-aligned offset) as the SBOX's input, and wires the SBOX's
# 8-bit output to the 8 positions output_positions_pattern (+ the same offset). Repeating this
# self.number_of_sboxes = word_size // 8 times, with each chunk's 9-position window overlapping
# its neighbour by exactly 1 bit, covers the whole word.
# fmt: off
SBOX = [
...
class SimonSboxBlockCipher(Cipher):
    ...
    This is functionally equivalent to :py:class:`SimonBlockCipher`: it replaces the AND of two
    rotated copies of x in the round function with a 512-entry SBOX lookup table (see the module-
    level comment above ``SBOX`` and the ``f`` method for exactly what that table computes and why
    it is addressed by 9 -- not 8 -- input bits).
    ...
    Beaulieu, R., Shors, D., Smith, J., Treatman-Clark, S., Weeks, B., & Wingers, L. (2013). The SIMON and SPECK
    Families of Lightweight Block Ciphers. Cryptology ePrint Archive, Report 2013/404.
    https://eprint.iacr.org/2013/404 [BSSTWW2013]_.

    The default (block=32, key=64) test vector used in ``simon_sbox_block_cipher_test.py`` (and in
    ``simon_block_cipher_test.py``) -- key ``0x1918111009080100``, plaintext ``0x65656877``, ciphertext
    ``0xc69be9bb`` -- is quoted verbatim from Appendix B of the paper above (the ``Simon32/64`` entry); it was not
    independently generated by this project.
    ...
    def f(self, x):
        ...
        # part, (x <<< 1) & (x <<< 8), 8 bits at a time; the XOR with (x <<< 2) is applied below by
        # a plain XOR component, mirroring the plaintext formula exactly.
        #
        # input_positions_pattern has 9 entries (not 8): each 8-bit SBOX call needs 9 bits of x,
        # since two neighbouring output bits' windows overlap by 1 bit (see the module-level
        # comment on SBOX). output_positions_pattern has the corresponding 8 output positions.
        # SBOX itself has 512 = 2**9 entries -- it is genuinely addressed by all 9 selected bits,
        # nothing is truncated, wrapped, or otherwise discarded.
