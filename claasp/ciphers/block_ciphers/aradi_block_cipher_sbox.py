...
class AradiBlockCipherSBox(Cipher):
    ...
    def l_function(self, xy_id_links, xy_input_bits, round_index):
        ...
        left_part = self.add_XOR_component(xy_id_links[:16] + [rot_x_a, rot_y_c],
                                           xy_input_bits[:16] + [list(range(16)), list(range(16))], 16).id
        right_part = self.add_XOR_component(xy_id_links[16:] + [rot_y_a, rot_x_b],
                                            xy_input_bits[16:] + [list(range(16)), list(range(16))], 16).id
    ...
    def m_function(self, i, j, xy_id_links, xy_input_bits):
        ...
        left_part = self.add_XOR_component(xy_id_links + [rot_i_y, rot_j_x],
                                           [x_inds] + [list(range(32)) for _ in range(2)], 32).id
