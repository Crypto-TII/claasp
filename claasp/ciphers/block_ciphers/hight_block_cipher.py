...
class HightBlockCipher(Cipher):
    ...
    def __init__(
        self, block_bit_size=64, key_bit_size=128, number_of_rounds=0, sub_keys_zero=False, transformations_flag=True
    ):
    ...
    def add_intermediate_output_components(self, round_key, internal_state, number_of_rounds, round_i):
        self.add_round_key_output_component(
            ...
            [get_ith_word(self.word_size, 0, round_key[k].id, round_key[k].input_bit_positions) for k in range(4)],
            4 * self.word_size,
        )
        ...
        if round_i == number_of_rounds - 1:
            self.add_cipher_output_component(
                [internal_state[k].id for k in range(8)], first_input_bit_positions, self.block_bit_size
            )
        else:
    ...
    def round_function(self, internal_state, round_key):
        def f0(x):
            rot_1 = self.add_rotate_component(
                [x.id], [get_ith_word(self.word_size, 0, x.id, x.input_bit_positions)], self.word_size, -1
            )

            rot_2 = self.add_rotate_component(
                [x.id], [get_ith_word(self.word_size, 0, x.id, x.input_bit_positions)], self.word_size, -2
            )

            rot_7 = self.add_rotate_component(
        ...
        def f1(x):
            rot_1 = self.add_rotate_component(
                [x.id], [get_ith_word(self.word_size, 0, x.id, x.input_bit_positions)], self.word_size, -3
            )

            rot_2 = self.add_rotate_component(
                [x.id], [get_ith_word(self.word_size, 0, x.id, x.input_bit_positions)], self.word_size, -4
            )
