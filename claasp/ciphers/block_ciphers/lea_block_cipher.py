...
def init_input(number_of_words, cipher_input, reorder_input_and_output):
    ...
    for i in range(number_of_words):
        if reorder_input_and_output:
            input_bit_positions = [list(itertools.chain(*[get_ith_word(8, 4 * i + 4 - j - 1) for j in range(4)]))]
        else:
...
class LeaBlockCipher(Cipher):
    ...
    def __init__(self, block_bit_size=128, key_bit_size=192, number_of_rounds=0, reorder_input_and_output=True):
        ...
        for round_i in range(number_of_rounds):
            ...
            if key_bit_size == 128:
                ...
                round_key_state, round_key = self.get_ith_key128(
                    round_key_state, round_i, number_of_operations, delta_constant, left_rotations_list
                )
            elif key_bit_size == 192:
                delta_constant = KEY_SCHEDULE_CONSTANTS[round_i % 6]
                number_of_operations = 6
                round_key_state, round_key = self.get_ith_key192(
                    round_key_state, round_i, number_of_operations, delta_constant, left_rotations_list
                )
            elif key_bit_size == 256:
                delta_constant = KEY_SCHEDULE_CONSTANTS[round_i % 8]
                number_of_operations = 6
                round_key_state, round_key = self.get_ith_key256(
                    round_key_state, round_i, number_of_operations, delta_constant, left_rotations_list
                )
            else:
            ...
            self.add_intermediate_output_components(
                round_key, internal_state, number_of_rounds, round_i, reorder_input_and_output
            )

    def add_intermediate_output_components(
        self, round_key, internal_state, number_of_rounds, round_i, reorder_input_and_output
    ):
        self.add_round_key_output_component(
            [round_key[k].id for k in range(6)], [get_ith_word(self.word_size, 0) for _ in range(6)], 6 * self.word_size
        )
        ...
        if round_i == number_of_rounds - 1:
            ...
            self.add_cipher_output_component(
                [internal_state[k].id for k in range(4)], first_input_bit_positions, self.block_bit_size
            )
        else:
    ...
    def get_ith_key128(self, key, round_i, number_of_operations, delta_constant, left_rotations_list):
        for operation in range(number_of_operations):
            ...
            rot_c = self.add_rotate_component(
                [mod_j.id], [get_ith_word(self.word_size, 0)], self.word_size, left_rotations_list[operation]
            )
            key[operation] = rot_c
    ...
    def round_function(self, internal_state, round_key):
        def word_function(i0, i1, i2, i3, rot_value):
            ...
            rot_c = self.add_rotate_component([mod_1.id], [get_ith_word(self.word_size, 0)], self.word_size, rot_value)

            return rot_c

        word_parameters = [[0, 0, 1, 1, -9], [1, 2, 2, 3, 5], [2, 4, 3, 5, 3]]
        new_state = []
