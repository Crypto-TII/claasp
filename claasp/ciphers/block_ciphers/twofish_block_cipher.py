...
class TwofishBlockCipher(Cipher):
    ...
    def __init__(self, key_length=128, number_of_rounds=16):
        ...
        super().__init__(family_name="twofish_block_cipher",
        ...
        self.MDS = [[0x01, 0xEF, 0x5B, 0x5B],
                    [0x5B, 0xEF, 0xEF, 0x01],
                    [0xEF, 0x5B, 0x01, 0xEF],
                    [0xEF, 0x01, 0xEF, 0x5B]]

        self.RS = [[0x01, 0xA4, 0x55, 0x87, 0x5A, 0x58, 0xDB, 0x9E],
                   [0xA4, 0x56, 0x82, 0xF3, 0x1E, 0xC6, 0x68, 0xE5],
                   [0x02, 0xA1, 0xFC, 0xC1, 0x47, 0xAE, 0x3D, 0x19],
                   [0xA4, 0x55, 0x87, 0x5A, 0x58, 0xDB, 0x9E, 0x03]]

        self.q_PERMUTATIONS = {0: [0xA9, 0x67, 0xB3, 0xE8, 0x04, 0xFD, 0xA3, 0x76, 0x9A, 0x92, 0x80, 0x78, 0xE4, 0xDD, 0xD1, 0x38,
                               ...
                               1: [0x75, 0xF3, 0xC6, 0xF4, 0xDB, 0x7B, 0xFB, 0xC8, 0x4A, 0xD3, 0xE6, 0x6B, 0x45, 0x7D, 0xE8, 0x4B,
        ...
        if self.key_padding_length != 0:
            ...
            master_key = self.add_permutation_component(
                    [INPUT_KEY], [list(range(self.key_block_size))],
                    self.key_block_size, list(range(self.key_block_size)))
        ...
        for i in range(4):
            X1 = self.add_constant_component(
                    32,
                    2 * i + 2 * i * 2 ** 8 + 2 * i * 2 ** 16 + 2 * i * 2 ** 24)
            X2 = self.add_constant_component(
                    32,
                    (2 * i + 1) + (2 * i + 1) * 2 ** 8 + (2 * i + 1) * 2 ** 16 + (2 * i + 1) * 2 ** 24)
            ...
            A1 = self.add_MODADD_component(
                    [A, B1.id],
                    [list(chain(range(24, 32), range(16, 24), range(8, 16), range(8))), list(range(32))],
                    32)
            keys_list[2 * i + 1] = A1
        ...
        for i in range(2):
            s0 = self.add_XOR_component(
                    ...
                    [list(chain(range(32 * 2 * i + 24, 32 * 2 * i + 32),
            ...
            s1 = self.add_XOR_component(
                    ...
                    [list(chain(range(32 * (2 * i + 1) + 24, 32 * (2 * i + 1) + 32),
        ...
        for round_number in range(number_of_rounds):
            #Key Schedule
            X1 = self.add_constant_component(32, (2 * round_number + 8)+ (2 * round_number + 8) * 2 ** 8 + (2 * round_number + 8) * 2 ** 16 + (2 * round_number + 8) * 2 ** 24)
            X2 = self.add_constant_component(32, (2 * round_number + 9) + (2 * round_number + 9) * 2 ** 8 + (2 * round_number + 9) * 2 ** 16 + (2 * round_number + 9) * 2 ** 24)
            A = self.h_function(X1.id, [M_e[k].id for k in range(self.key_k)], M_e_positions)
            B = self.h_function(X2.id, [M_o[k].id for k in range(self.key_k)], M_o_positions)
            keys_list[(2 * round_number + 8)] = self.add_MODADD_component(
                    ...
                    [list(chain(range(24, 32), range(16, 24), range(8, 16), range(8))),
                     list(chain(range(16, 24), range(8, 16), range(8), range(24, 32)))],
                    32)
            B1 = self.add_SHIFT_component([B], [list(chain(range(16, 24), range(8, 16), range(8), range(24, 32)))], 32, -1)
            A1 = self.add_MODADD_component([A, B1.id], [list(chain(range(24, 32), range(16, 24), range(8, 16), range(8))), list(range(32))], 32)
            ...
            FT = self.add_SHIFT_component(
                    [T1], [list(chain(range(24, 32), range(16, 24), range(8, 16), range(8)))],
                    32, -1)
            ...
            if round_number == number_of_rounds - 1:
                ...
                self.add_intermediate_output_component(
                        ...
                        self.cipher_block_size, "round_output")
                self.add_round()
    ...
    def h_function(self, X, L, L_bits):
        ...
        if self.key_k == 4:
            y4_j = X
            y_i[3] = [self.add_SBOX_component([y4_j], [list(range(8 * (3 - j), 8 * (4 - j)))],
                                              8, self.q_PERMUTATIONS[int(abs(j - 1.5))]) for j in range(4)]
            y3_j = self.add_XOR_component([y_i[3][j].id for j in range(4)] + [L[3]],
                                          [list(range(8)) for _ in range(4)] + [L_bits[3]], 32)
            y_i[2] = [self.add_SBOX_component([y3_j.id], [list(range(8 * j, 8 * (j + 1)))],
                                              8, self.q_PERMUTATIONS[1 - int(j / 2)]) for j in range(4)]
            y2_j = self.add_XOR_component([y_i[2][j].id for j in range(4)] + [L[2]],
                                          [list(range(8)) for _ in range(4)] + [L_bits[2]], 32)
            y_i[1] = [self.add_SBOX_component([y2_j.id], [list(range(8 * j, 8 * (j + 1)))],
                                              8, self.q_PERMUTATIONS[j % 2]) for j in range(4)]
            y1_j = self.add_XOR_component([y_i[1][j].id for j in range(4)] + [L[1]],
                                          [list(range(8)) for _ in range(4)] + [L_bits[1]], 32)

        elif self.key_k == 3:
            y3_j = X
            y_i[2] = [self.add_SBOX_component([y3_j], [list(range(8 * (3 - j), 8 * (4 - j)))],
                                                    8, self.q_PERMUTATIONS[1 - int(j / 2)]) for j in range(4)]
            y2_j = self.add_XOR_component([y_i[2][j].id for j in range(4)] + [L[2]],
                                          [list(range(8)) for _ in range(4)] + [L_bits[2]], 32)
            y_i[1] = [self.add_SBOX_component([y2_j.id], [list(range(8 * j, 8 * (j + 1)))], 8,
                                              self.q_PERMUTATIONS[j % 2]) for j in range(4)]
            y1_j = self.add_XOR_component([y_i[1][j].id for j in range(4)] + [L[1]],
                                          [list(range(8)) for _ in range(4)] + [L_bits[1]], 32)

        elif self.key_k == 2:
            y_i[1] = [self.add_SBOX_component([y2_j], [list(range(8 * (3 - j), 8 * (4 - j)))],
                                              8, self.q_PERMUTATIONS[j % 2]) for j in range(4)]
            y1_j = self.add_XOR_component([y_i[1][j].id for j in range(4)] + [L[1]],
                                          [list(range(8)) for _ in range(4)] + [L_bits[1]], 32)

        y_i[0] = [self.add_SBOX_component([y1_j.id], [list(range(8*j, 8*(j+1)))], 8,
                                          self.q_PERMUTATIONS[int(j / 2)]) for j in range(4)]
        y0_j = self.add_XOR_component([y_i[0][j].id for j in range(4)] + [L[0]],
                                      [list(range(8)) for _ in range(4)] + [L_bits[0]], 32)

        y = [self.add_SBOX_component([y0_j.id], [list(range(8 * j, 8 * (j + 1)))], 8,
                                     self.q_PERMUTATIONS[1 - (j % 2)]) for j in range(4)]

        z = self.add_mix_column_component([y[j].id for j in range(4)], [list(range(8)) for _ in range(4)],
                                          32, [self.MDS, self.state_MDS_polynomial, 8])
        return z.id
