...
class DESExactKeyLengthBlockCipher(Cipher):
    """
    ...
    EXAMPLES::
        ...
        sage: from claasp.ciphers.block_ciphers.des_exact_key_length_block_cipher import DESExactKeyLengthBlockCipher
        sage: cipher = DESExactKeyLengthBlockCipher()
        sage: cipher.print_as_python_dictionary_to_file("claasp/ciphers/"+cipher.file_name) # doctest: +SKIP

        sage: from claasp.ciphers.block_ciphers.des_exact_key_length_block_cipher import DESExactKeyLengthBlockCipher
        sage: des_cipher = DESExactKeyLengthBlockCipher()
        sage: key = 0x12695BC9B7B7F8
        sage: plaintext = 0x0123456789ABCDEF
        sage: ciphertext = 0x85E813540F0AB405
        sage: des_cipher.evaluate([key, plaintext]) == ciphertext
        True
    """
    ...
    def __init__(self, number_of_rounds=16, number_of_sboxes=8):
        ...
        self.key_block_size = number_of_sboxes * 7
        self.half_round_key_size = int(number_of_sboxes / 2) * 7
        self.nrounds = number_of_rounds
        self.sbox_bit_size = 6
        self.num_sboxes = number_of_sboxes

        super().__init__(
            family_name="des_exact_key_length_block_cipher",
        ...
        for round_number in range(number_of_rounds):
            ...
            if round_number == number_of_rounds - 1:
                ...
                self.add_cipher_output_component(
                    [output.id], [list(range(self.cipher_block_size))], self.cipher_block_size
                )
                state = output
            else:
                output = self.add_permutation_component(
