...
class AradiBlockCipherSBox(Cipher):
    ...
    def __init__(self, number_of_rounds=16):
        ...
        self.add_cipher_output_component([state], [list(range(128))], 128)
    ...
    def round_function(self, state, round_key, round_i):
        ...
        #sb_outputs[0][0]sb_outputs[1][0]sb_outputs[2][0]                     sb_outputs[0][1]sb_outputs[1][1]                  sb_outputs[0][2]sb_outputs[1][2]                 sb_outputs[0][3]sb_outputs[1][3]
        w_ = [sb_outputs[i] for i in range(32)]

        #print(len(w_+w_+w_+w_))
        w_input_bits = [[0] for _ in sb_outputs]

        self.add_intermediate_output_component(
            w_+w_+w_+w_,

                [[0] for _ in range(32)] +[[1] for _ in range(32)] + [[2] for _ in range(32)] + [[3] for _ in range(32)]
            ,
            128,
            f"sbox_output_{round_i}"
        )
