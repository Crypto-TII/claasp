...
class QARMAv2BlockCipher(Cipher):
    ...
    def __init__(self, number_of_rounds=10, number_of_layers=1, key_bit_size=128, tweak_bit_size=128):
        ...
        state = self.last_round_end(state, key_state)
    ...
    def direct_round(self, state, key_state, tweak_state, tweak_permutation, constants_states, round_number):
            ...
            if len(key_state[round_number % 2][0]) == 1:
                id_links = [
                    [
                        ...
                        constants_states[(round_number - 1) * self.number_of_layers + i // 16],
                    ]
                ...
                id_links = [
                    [
                        ...
                        key_state[round_number % 2][0][0],
                        tweak_state[round_number % 2][0][0],
                        constants_states[(round_number - 1) * self.number_of_layers + i // 64],
                    ]
                    for i in range(self.num_sboxes // 2)
                ]
                id_links.extend(
                    [
                        [
                        ...
                        for i in range(self.num_sboxes // 2, self.num_sboxes)
                    ]
                )
                bit_positions = [
                    [
        ...
        if self.number_of_layers == 2 and (self.nrounds - round_number) % 2 == 0:
            round_output = self.add_round_output_component(
                [sboxed_state[i] for i in self.exchange_rows_shuffle],
                [list(range(4)) for _ in range(self.num_sboxes)],
                self.cipher_block_size,
            ).id
        else:
            round_output = self.add_round_output_component(
                sboxed_state, [list(range(4)) for _ in range(self.num_sboxes)], self.cipher_block_size
            ).id
    ...
    def reflector(self, state, key_state):
        ...
        bit_positions = [
            [
                list(
                    range(
                        4 * self.state_shuffle[i % 16] + 64 * (i // 16),
                        4 * self.state_shuffle[i % 16] + 4 + 64 * (i // 16),
                    )
                ),
                list(range(4 * i, 4 * i + 4)),
            ]
    ...
    def inverse_round(self, state, key_state, tweak_state, tweak_permutation, constants_states, round_number):
        ...
        if round_number != 1:
            round_output = self.add_round_output_component(
                masked_state, [list(range(4)) for i in range(self.num_sboxes)], self.cipher_block_size
            ).id
    ...
    def last_round_end(self, state, key_state):
        ...
        round_output = self.add_round_output_component(
            [masked_state[i] for i in range(self.num_sboxes)],
