...
class SHA2HashFunction(Cipher):
    ...
    def __init__(self, output_bit_size=256, number_of_rounds=64):
        ...
        self.add_cipher_output_component([cipher_output_state[i].id for i in range(numbers_of_words_in_digest)],
    ...
    def add_rotate_component_sha2(self, component, amount):
        return self.add_rotate_component([component.id],
                                         ...
                                         self.word_size, amount)

    def add_round_output_component_sha2(self, a, b, c, d, e, f, g, h):
        return self.add_round_output_component([a.id, b.id, c.id, d.id, e.id, f.id, g.id, h.id],
    ...
    def compute_bsig0_bsig1(self, component_0, component_1):
        ...
        if self.output_bit_size in (224, 256):
            ...
            BSIG0 = self.add_XOR_component([ROTR_2.id, ROTR_13.id, ROTR_22.id],
            ...
            BSIG1 = self.add_XOR_component([ROTR_6.id, ROTR_11.id, ROTR_25.id],
                                           [list(range(self.word_size)) for _ in range(3)],
                                           self.word_size)
            ...
            BSIG0 = self.add_XOR_component([ROTR_28.id, ROTR_34.id, ROTR_39.id],
                                           [list(range(self.word_size)) for _ in range(3)],
                                           self.word_size)
            ...
            BSIG1 = self.add_XOR_component([ROTR_14.id, ROTR_18.id, ROTR_41.id],
                                           [list(range(self.word_size)) for _ in range(3)],
                                           self.word_size)
    ...
    def compute_maj(self, x, y, z):
        ...
        return self.add_XOR_component([x_AND_y.id, y_AND_z.id, x_AND_z.id],
                                      [list(range(self.word_size)) for _ in range(3)],
                                      self.word_size)
    ...
    def compute_ssig0_ssig1(self, W, t):
        ...
        if self.output_bit_size in (224, 256):
            ...
            SSIG0 = self.add_XOR_component([ROTR_7.id, ROTR_18.id, SHR_3.id],
                                           [list(range(self.word_size)) for _ in range(3)],
                                           self.word_size)
            ...
            SSIG1 = self.add_XOR_component([ROTR_17.id, ROTR_19.id, SHR_10.id],
                                           [list(range(self.word_size)) for _ in range(3)],
                                           self.word_size)
            ...
            SSIG0 = self.add_XOR_component([ROTR_1.id, ROTR_8.id, SHR_7.id],
                                           [list(range(self.word_size)) for _ in range(3)],
                                           self.word_size)
            ...
            SSIG1 = self.add_XOR_component([ROTR_19.id, ROTR_61.id, SHR_6.id],
                                           [list(range(self.word_size)) for _ in range(3)],
                                           self.word_size)
    ...
    def round_function(self, a, b, c, d, e, f, g, h, Kt, W):
        ...
        T1 = self.add_MODADD_component([h.id, BSIG1_e.id, CH_e_f_g.id, Kt.id, W.id],
    ...
    def schedule(self, W, t):
        ...
        Wt = self.add_MODADD_component([W15.id, W2.id, W[t - 7].id, W[t - 16].id],
                                       [list(range(self.word_size)), list(range(self.word_size))] +
                                       W[t - 7].input_bit_positions + W[t - 16].input_bit_positions,
                                       self.word_size)
