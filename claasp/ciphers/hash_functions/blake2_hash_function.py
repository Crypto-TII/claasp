...
class Blake2HashFunction(Cipher):
    ...
    def __init__(self, block_bit_size=1024, state_bit_size=1024, number_of_rounds=0, word_size=64, permutations=None,
                 rot_amounts=None):
        ...
        for round_number in range(number_of_rounds):
            ...
            # STATE TRANSFORMATION
            state_word_ids, state_word_ranges = self.column_step(data_word_ids, data_word_ranges, state_word_ids,
                                                                 state_word_ranges, round_number)

            state_word_ids, state_word_ranges = self.diagonal_step(data_word_ids, data_word_ranges, state_word_ids,
                                                                   state_word_ranges, round_number)
    ...
    def column_step(self, data_word_ids, data_word_ranges, state_word_ids, state_word_ranges, r):
        ...
        self.add_intermediate_output_component(new_state_word_ids, new_state_word_ranges, self.block_bit_size,
                                               'column_step_output')
    ...
    def diagonal_step(self, data_word_ids, data_word_ranges, state_word_ids, state_word_ranges, r):
        ...
        self.add_intermediate_output_component(new_state_word_ids, new_state_word_ranges, self.block_bit_size,
                                               'diagonal_step_output')

        return new_state_word_ids, new_state_word_ranges

    def state_transformation(self, data_word_ids, data_word_ranges, state_word_ids, state_word_ranges,
                             word_indexes, m0, m1):
        ...
        for i in range(self.n):
            ...
            xor = self.add_XOR_component([new_state_id[(j - 1) % self.n], new_state_id[j]],
                                         [new_state_range[(j - 1) % self.n], new_state_range[j]],
                                         self.word_size).id

            new_state_id[(j - 1) % self.n] = self.add_rotate_component([xor], [list(range(self.word_size))],
                                                                       self.word_size, self.rot_amounts[i]).id
