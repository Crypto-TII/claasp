...
class BlakeHashFunction(Cipher):
    ...
    - ``number_of_rounds`` -- **integer** (default: `0`); number of rounds of the hash. The cipher uses the
    ...
    def state_transformation(
        ...
        for i in range(self.n):
            ...
            new_state_id[(j - 1) % self.n] = self.add_rotate_component(
                [xor_id], [list(range(self.word_size))], self.word_size, self.rot_amounts[i]
            ).id
