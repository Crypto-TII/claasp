...
class GastonPermutation(Cipher):
    ...
    def __init__(self, number_of_rounds=12):
        ...
        for r in range(12 - number_of_rounds, 12):
            ...
            self.add_round()
            # round function
            state = self.gaston_round_function(state, gaston_rc[r])
            # round output
            inputs = []
            for i in range(GASTON_NROWS):
    ...
    def gaston_rho_east(self, state):
        for i in range(GASTON_NROWS):
            self.add_rotate_component(state[i].id, state[i].input_bit_positions, WORD_SIZE, -GASTON_e[i])
    ...
    def gaston_theta(self, state):
        ...
        for i in range(GASTON_NROWS):
            inputs_id, inputs_pos = get_inputs_parameter([state[i], P])
            self.add_XOR_component(inputs_id, inputs_pos, WORD_SIZE)
            state[i] = ComponentState([self.get_current_component_id()], [list(range(WORD_SIZE))])
        return state
    ...
    def gaston_rho_west(self, state):
        for i in range(GASTON_NROWS):
            self.add_rotate_component(state[i].id, state[i].input_bit_positions, WORD_SIZE, -GASTON_w[i])
            state[i] = ComponentState([self.get_current_component_id()], [list(range(WORD_SIZE))])
        return state
