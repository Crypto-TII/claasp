...
class SubterraneanPermutation(Cipher):
    ...
    def __init__(self, number_of_rounds=1, version=Version.V1):
        ...
        state = ComponentState([INPUT_PLAINTEXT], [list(range(self.state_bit_size))])
        ...
        for r in range(rounds):
            ...
            state = self._step_3(state)
            if version == Version.V1:
                state = self._step_4(state, key)
            state = self._step_5(state)
    ...
    def _rotate(self, state: ComponentState, size: int, parameter: int) -> ComponentState:
        ids, bits = get_inputs_parameter([state])
        self.add_rotate_component(ids, bits, size, parameter)
        return ComponentState([self.get_current_component_id()], [list(range(size))])

    def _step_1(self, state: ComponentState) -> ComponentState:

        a2 = self._rotate(state, self.state_bit_size, -2)
        ...
        a1 = self._rotate(state, self.state_bit_size, -1)
    ...
    def _step_2(self, state: ComponentState) -> ComponentState:
        # negation of the first bit
        neg = self.add_not_component([state.id[0]], [[state.input_bit_positions[0][0]]], 1)

        return ComponentState([neg.id, *state.id], [[0], state.input_bit_positions[0][1:]])

    def _step_3(self, state: ComponentState) -> ComponentState:
        a3 = self._rotate(state, self.state_bit_size, -3)
        a8 = self._rotate(state, self.state_bit_size, -8)

        ids, bits = get_inputs_parameter([state, a3, a8])
        self.add_xor_component(ids, bits, self.state_bit_size)

        return ComponentState([self.get_current_component_id()], [list(range(self.state_bit_size))])

    def _step_4(self, state: ComponentState, key: ComponentState) -> ComponentState:
    ...
    def _step_5(self, state: ComponentState) -> ComponentState:
    ...
    def _chi(self, state: ComponentState) -> ComponentState:
        ...
        s2 = self._rotate(state, self.state_bit_size, -2)
