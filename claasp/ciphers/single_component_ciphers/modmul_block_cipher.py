...
class ModmulBlockCipher(SingleComponentCipher):
    """
    Return a single-round cipher made of a single modular-multiplication component.

    INPUT:

    - ``word_bit_size`` -- **integer** (default: `4`); bit size of each operand and of the output
    - ``number_of_inputs`` -- **integer** (default: `2`); number of operands
    - ``modulus`` -- **integer** (default: `None`); optional modulus override

    EXAMPLES::

        sage: from claasp.ciphers.single_component_ciphers.modmul_block_cipher import ModmulBlockCipher
        sage: modmul = ModmulBlockCipher(word_bit_size=4, number_of_inputs=2, modulus=16)
        sage: modmul.type
        'block_cipher'

        sage: modmul.component_from(0, 0).id
        'modmul_0_0'

        sage: modmul.evaluate([3, 5])
        15
    """
    ...
    def __init__(self, word_bit_size=4, number_of_inputs=2, modulus=None):
        ...
        super().__init__(
            family_name="modmul_block_cipher",
