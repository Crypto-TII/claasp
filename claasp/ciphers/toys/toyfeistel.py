...
PARAMETERS_CONFIGURATION_LIST = [{"block_bit_size": 8, "self.key_bit_size": 8, "number_of_rounds": 5}]
...
class ToyFeistel(Cipher):
    ...
    def __init__(
        ...
        for round_i in range(1, number_of_rounds + 1):
            ...
            round_key = self.add_round_key_output_component(
                [key], [list(range(self.block_bit_size // 2))], self.block_bit_size // 2
            ).id
            state = self.round_function(state, round_key)
            key = self.update_key(key, round_i)
        self.add_cipher_output_component(
            [state, state],
            [list(range(self.block_bit_size // 2, self.block_bit_size)), list(range(self.block_bit_size // 2))],
            self.block_bit_size,
        )
    ...
    def update_key(self, k, i):
        ...
        updated_key = self.add_intermediate_output_component(
            [xor0, xor1],
            [list(range(self.key_bit_size // 2)), list(range(self.key_bit_size // 2))],
            self.key_bit_size,
            "updated_key",
        )
    ...
    def round_function(self, state, key):
        ...
        round_output = self.add_round_output_component(
            [state, new_right_word], [right_half_positions, list(range(self.block_bit_size // 2))], self.block_bit_size
        ).id
        return round_output
